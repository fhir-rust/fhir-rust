...
- **fhir-postgresql-map** — the relational map model (a compact, serialized
...
- **fhir-postgresql-gen** — reads a FHIR specification package
...
- **fhir-postgresql-store** — tokio-postgres + deadpool. Transactional writes with
...
- **fhir-postgresql-server** — axum. The FHIR RESTful API, bundle processing,
  generated CapabilityStatements, request ids, metrics.
- **fhir-postgresql** — the CLI binary tying it together.
