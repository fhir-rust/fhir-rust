...
files' unrelated lines, and a third (targeting `fhir-sqlite` alone)
duplicated part of the first. `cargo check`/`clippy`/`fmt` clean; the live
suite (including the hash-chain tests, which exercise both crates
directly) re-run against a real PostgreSQL container, all green. No API
change. Released as a patch rather than bundled into 0.6.0 because that
version was already published — `O10.11` requires a published version to
match its source, and `check-published-match.sh` cannot see this specific
kind of change (a workspace-inherited dependency requirement is invisible
to its `Cargo.toml.orig`-only comparison — a gap worth fixing separately,
not something this release papers over).
...
`fhir-postgresql init --upgrade` for the new digest, tag, and counter-signature
columns; existing rows keep verifying, with the new chains reported as
beginning where their first digest appears.


**Changed — hash chains are computed by fhir-postgresql, not by PostgreSQL.** The
...
`FHIR_POSTGRESQL_CHAIN_KEY` and never written to the database or logged. It sits
...
additive: retired keys stay loadable via `FHIR_POSTGRESQL_CHAIN_KEYS_RETIRED` and
...
**Added — `fhir-postgresql chain-witness` and automatic checkpoints.** A tag proves a
...
fhir-postgresql cannot enforce that and does not claim to.
...
version of fhir-postgresql that had already shipped.
...
without `--allow-insecure-db`. fhir-postgresql's `require` validates the server
...
accents; fhir-postgresql shipped `ILIKE`, which ignores only case, so `family=muller`
...
too low" was "rebuild it". `--pool-size` beats `FHIR_POSTGRESQL_POOL_SIZE`, so a
...
`fhir_postgresql_request_latency_micros_total`, which with a request count gives only
...
`fhir_postgresql_request_latency_seconds` is now a Prometheus histogram over the
...
- Pool size is configurable via `FHIR_POSTGRESQL_POOL_SIZE` instead of compiled in.
- `crates/fhir-postgresql-store/tests/concurrency.rs`: torn reads, racing conditional
  creates, and racing `If-Match` updates (T11.6).

**Added — the audit trail (M3.15–M3.17, §12)**

fhir-postgresql still does not authenticate; the perimeter does. But "authentication
...
- Every **read** appends a disclosure record to `fhir_postgresql_access_log`: read,
...
- History is **tamper-evident**: a per-resource SHA-256 chain
  ...
  `now()` and cannot race the read of the previous hash. `fhir-postgresql
...
The restriction was never fhir-postgresql's: the published `fhir` 1.2.0 could not
...
fhir-postgresql's floor moves to `fhir = "1.2.1"` with `features = ["r3", "r4"]`, and
...
- The CapabilityStatement now declares `versioning`, `readHistory`,
  ...
  `security` block stating plainly that fhir-postgresql verifies no identity (A7.12).
- CI gains a supply-chain job (cargo-deny, CycloneDX SBOM) and `deny.toml`
  (O10.10).
- `book/src/trust-boundary.md`: what fhir-postgresql guarantees, what the deployment
...
- FHIR REST server (`fhir-postgresql serve`): every installed version mounted,
