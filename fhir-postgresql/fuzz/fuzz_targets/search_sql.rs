...
fuzz_target!(|data: &[u8]| {
    ...
    let sort: Vec<fhir_postgresql_store::search::SortKey> = params
        .iter()
        .take(2)
        .map(|(k, _)| fhir_postgresql_store::search::SortKey {
            code: k.clone(),
            descending: k.starts_with('-'),
        })
        .collect();

    let Ok(query) = fhir_postgresql_store::search::build_search_sql(
        map,
        rm,
        &params,
        50,
