...
cargo test -p fhir-postgresql-store --test live -- --nocapture --test-threads=1
```
...
1. `FHIR_POSTGRESQL_SPEC_DIR`, if set.
...
Set `FHIR_POSTGRESQL_SPEC_DIR` to point somewhere else. The tests themselves fall back to
...
| Variable | Meaning |
|---|---|
| `FHIR_POSTGRESQL_TEST_DB` | Set by `dsn`. Without it the live tests skip. |
| `FHIR_POSTGRESQL_SPEC_DIR` | FHIR definitions directory. |
| `FHIR_POSTGRESQL_CORPUS_DIR` | Example corpus directory. |
| `FHIR_POSTGRESQL_TEST_CORPUS_LIMIT` | Examples per version (default 400; use 10–25 for a fast loop). |
| `FHIR_POSTGRESQL_CHAIN_KEY` | Hex key enabling the keyed-MAC audit tests. |
| `FHIR_POSTGRESQL_BENCH` | Set to a row count to enable the benchmark test. |
...
**Port already in use** — something else is on `15432`, often a
