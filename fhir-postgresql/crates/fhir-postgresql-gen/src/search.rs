...
pub fn add_norm_columns(map: &mut RelMap) {
    ...
    use fhir_postgresql_map::model::{ColTy, Column};
...
pub fn add_adjunct_columns(map: &mut RelMap) {
    use crate::names::Registry;
    use fhir_postgresql_map::model::{Adjuncts, ColTy, Column, TableKind};

    if !fhir_postgresql_map::ddl::TEXT_ADJUNCTS {
        return;
    }
    ...
    for rm in map.resources.values_mut() {
        ...
        for ((table, col), (bounded, digest)) in keys {
            ...
            if !fhir_postgresql_map::ddl::needs_adjunct(src.ty) {
                continue;
            }
...
#[cfg(test)]
mod adjunct_rules {
    use fhir_postgresql_map::ddl::{TEXT_ADJUNCTS, needs_adjunct};
    use fhir_postgresql_map::model::ColTy;
