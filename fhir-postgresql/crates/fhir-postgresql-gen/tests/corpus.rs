...
//! Set FHIR_POSTGRESQL_CORPUS_DIR to a directory containing r5/, r4/, stu3/
...
use fhir_postgresql_map::shred::{SqlVal, shred};
use serde_json::Value;

/// Where the FHIR example corpus lives, if it has been fetched.
///
/// `FHIR_POSTGRESQL_CORPUS_DIR` first, then `corpus/` beside the workspace root —
...
fn corpus_root() -> Option<PathBuf> {
    if let Ok(dir) = std::env::var("FHIR_POSTGRESQL_CORPUS_DIR") {
...
///
/// `FHIR_POSTGRESQL_SPEC_DIR` first, then `spec-cache/` beside the workspace root, to
...
fn to_recon(rm: &fhir_postgresql_map::ResourceMap, out: &fhir_postgresql_map::ShredOut) -> ReconIn {
...
fn roundtrip_full_corpus() {
    ...
    for (cdir, sdir, schema) in [("stu3", "r3", "r3"), ("r4", "r4", "r4"), ("r5", "r5", "r5")] {
        ...
        let map = fhir_postgresql_gen::generate(&defs, schema).expect("generate");
    ...
    assert!(
        ...
        if total == 0 {
            "the corpus directory exists but holds no stu3/r4/r5 examples: \
             the fetch did not happen. Fetch it, or unset FHIR_POSTGRESQL_CORPUS_DIR \
             to skip this test."
                .to_string()
        } else {
