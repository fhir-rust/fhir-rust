...
//!
//! FHIR_POSTGRESQL_PROPTEST_CASES overrides the case count (default 500 locally;
//! CI runs 10000).

use std::collections::HashMap;

use fhir_postgresql_map::model::{Elem, ElemKind, Prim, ResourceMap};
...
fn to_recon(rm: &ResourceMap, out: &fhir_postgresql_map::ShredOut) -> ReconIn {
...
fn property_roundtrip_random_resources() {
    let spec = std::env::var("FHIR_POSTGRESQL_SPEC_DIR")
    ...
    let cases: u64 = std::env::var("FHIR_POSTGRESQL_PROPTEST_CASES")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(500);
    let map = fhir_postgresql_gen::generate(&defs, "r5").expect("generate");
