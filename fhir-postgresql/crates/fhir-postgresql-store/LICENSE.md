...
This applies to every crate in the repository — all three families. The SPDX
expression `MIT OR Apache-2.0 OR BSD-3-Clause OR GPL-2.0-only OR GPL-3.0-only`
is declared identically in all 32 `[package]` manifests, and `OR` means the
recipient chooses; no obligation from one option carries into another.
