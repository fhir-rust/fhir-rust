...
//!
//! With `FHIR_POSTGRESQL_CHAIN_KEY` set, each link is an HMAC rather than a bare hash:
...
impl ChainKey {
    ...
    /// Read from `FHIR_POSTGRESQL_CHAIN_KEY` (hex) and `FHIR_POSTGRESQL_CHAIN_KEY_ID`.
    ///
    ...
    pub fn from_env() -> Result<Option<Self>, String> {
        let Ok(hex) = std::env::var("FHIR_POSTGRESQL_CHAIN_KEY") else {
            return Ok(None);
        };
        let id = std::env::var("FHIR_POSTGRESQL_CHAIN_KEY_ID").unwrap_or_else(|_| "k1".to_string());
...
impl KeyRing {
    ...
    ///
    /// `FHIR_POSTGRESQL_CHAIN_KEY` (hex) with optional `FHIR_POSTGRESQL_CHAIN_KEY_ID` is the
    /// signing key. `FHIR_POSTGRESQL_CHAIN_KEYS_RETIRED` holds `id=hex` pairs,
    ...
    pub fn from_env() -> Result<Self, String> {
        ...
        if let Ok(retired) = std::env::var("FHIR_POSTGRESQL_CHAIN_KEYS_RETIRED") {
...
mod tests {
    ...
    fn a_group_readable_key_file_is_refused() {
        ...
        let dir =
            std::env::temp_dir().join(format!("fhir-postgresql-keytest-{}", std::process::id()));
    ...
    fn an_unreadable_retired_key_is_an_error() {
        let missing = std::path::PathBuf::from("/nonexistent/fhir-postgresql/retired.key");
