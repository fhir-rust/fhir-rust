...
cargo test -p fhir-oracle-store --test live -- --nocapture --test-threads=1
```
...
Set `FHIR_ORACLE_SPEC_DIR` to point somewhere else. The tests themselves fall back to
...
| Variable | Meaning |
|---|---|
| `FHIR_ORACLE_TEST_DSN` | Set by `dsn`. Without it the live tests skip. |
