//! DDL emission — **NOT YET PORTED TO ORACLE.**
//!
//! This file is still the MySQL emitter it was forked from. It compiles and its
//! unit tests pass, but every one of those tests asserts *MySQL* behaviour, so a
//! green suite here says nothing about Oracle. The tests that make MySQL-specific
//! claims are `#[ignore]`d with that reason attached rather than left to imply a
//! conformance this port has not earned.
//!
//! What has to change, and why none of it is mechanical:
//!
//! - **Types.** `VARCHAR2(n CHAR)` and `CLOB`, not `VARCHAR`/`TEXT`; `NUMBER`
//!   for integers; `TIMESTAMP(6)`. Oracle had no boolean column type before 23ai,
//!   so `ColTy::Bool` becomes `NUMBER(1)` with a CHECK unless 23ai is required.
//! - **`VARCHAR2` maxes at 4000 bytes** (32767 with extended types enabled, which
//!   is a database-level setting a schema cannot assume). Anything longer is a
//!   `CLOB`, and a `CLOB` cannot be indexed or compared with `=` the way text is
//!   — a sharper version of the constraint the SQL Server port hit.
//! - **No `IF NOT EXISTS`** on anything. Idempotence is a `BEGIN … EXCEPTION
//!   WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;` wrapper, which
//!   makes every statement a PL/SQL block.
//! - **Identifiers were 30 bytes before 12.2**, 128 after. `PG_MAX_IDENT = 63`
//!   is safe for 12.2+, and the port must declare that floor rather than inherit
//!   the assumption silently.
//! - **Triggers** use `RAISE_APPLICATION_ERROR(-20001, …)`, and the erasure flag
//!   has no session-variable equivalent — `SYS_CONTEXT` with an application
//!   context is the analogue, and it needs a context package created up front.
//! - **`ords`** stays a text image, but `RAW(n)`/`VARCHAR2` rather than
//!   `VARBINARY`.
//!
//! Tracked as an open task; nothing here should be read as an Oracle schema.
...
pub fn ddl_in(map: &RelMap, schema: &str) -> Vec<String> {
    ...
    out.push(format!(
        "CREATE TABLE `{s}`.`fhir_oracle_meta` (`key` VARCHAR(191) NOT NULL PRIMARY KEY, `value` TEXT NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
    ));
...
fn access_log_table(s: &str) -> String {
    format!(
        "CREATE TABLE IF NOT EXISTS `{s}`.`fhir_oracle_access_log` (\n\
...
fn countersign_table(s: &str) -> String {
    format!(
        "CREATE TABLE IF NOT EXISTS `{s}`.`fhir_oracle_countersign` (\n\
         \x20 `seq` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,\n\
         ...
         \x20 UNIQUE KEY `fhir_oracle_countersign_uq` (`rtype`, `id`, `version_id`, `row_mac`)\n\
         ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
    )
}
...
fn access_log_indexes(s: &str) -> Vec<String> {
    ...
    vec![
        format!(
            "CREATE INDEX `fhir_oracle_access_log_subject_ix` ON `{s}`.`{t}` (`rtype`, `id`, `ts`)"
        ),
        format!(
            "CREATE INDEX `fhir_oracle_access_log_actor_ix` ON `{s}`.`{t}` (`actor`(191), `ts`)"
        ),
        format!("CREATE INDEX `fhir_oracle_access_log_ts_ix` ON `{s}`.`{t}` (`ts`)"),
    ]
}
...
/// `@fhir_oracle_erasure` is set, which is how `fhir-oracle purge` performs a
    ...
    vec![
        ...
        format!(
            "CREATE TRIGGER `{s}`.`{del}` BEFORE DELETE ON `{s}`.`{table}` FOR EACH ROW \
             BEGIN \
             IF COALESCE(@fhir_oracle_erasure, '') <> 'on' THEN \
             SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = \
...
mod tests {
    ...
    #[test]
    #[ignore = "inherited MySQL assertions; this port has no Oracle DDL yet"]
    ...
    fn append_only_triggers_drop_before_create_and_gate_delete() {
        ...
        assert!(!t[1].contains("fhir_oracle_erasure"));
        assert!(t[3].contains("@fhir_oracle_erasure"));
        for stmt in [&t[1], &t[3]] {
