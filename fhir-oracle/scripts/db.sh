...
it made every local run look like evidence (F-06). Spec O10.12 forbids it.

What does work without a database:

  cargo test --workspace      unit and round-trip tests, no engine needed

See spec/14-oracle-dialect.md for the decisions that unblock this.
MSG
  exit 1
  ;;
*)
