...
Ground-up rewrite of fhir-oracle: fully normalized relational storage of FHIR
R3/R4/R5 in Oracle Database. The REST server is a separate crate — `fhir-loco` — and
...
- **D2 — All resource types, three versions.** R5 5.0.0 (default), R4 4.0.1,
  R3 3.0.2, each complete, each in its own Oracle user (`r5`/`r4`/`r3`).
...
- **D13 — Auth is perimeter, not core.** The server implements no
  ...
  **Amended by D15:** the perimeter authenticates, but fhir-oracle must still
  *record* who acted.
- **D14 — Workspace layout.** One cargo workspace:
  `fhir-oracle-map` (relational map types + generic shred/reconstruct engine),
  `fhir-oracle-gen` (spec → DDL + map), `fhir-oracle-store` (Oracle Database layer:
  init/load/search/history). **There is no `fhir-oracle-server` and no
  `fhir-oracle` CLI binary** — this decision was the ancestor project's, and the
...
- **D15 — Attribution is core, even though authentication is not.** D13
  ...
  history. fhir-oracle accepts a principal from a trusted proxy (PR12.1–PR12.3)
