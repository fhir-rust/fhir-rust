...
> tense long after F-68, and pointed readers to the *Unreleased* section
...
`fhir-oracle init --upgrade` for the new digest, tag, and counter-signature
...
since **F-68** the store writes and verifies chains live.) The
...
`FHIR_ORACLE_CHAIN_KEY` and never written to the database or logged. It sits
...
additive: retired keys stay loadable via `FHIR_ORACLE_CHAIN_KEYS_RETIRED` and
...
**Added — `fhir-oracle chain-witness` and automatic checkpoints.** A tag proves a
...
fhir-oracle cannot enforce that and does not claim to.
...
version of fhir-oracle that had already shipped.
...
`fhir-oracle` has no connector to fix, and never had one. This entry is
...
without `--allow-insecure-db`. fhir-oracle's `require` validates the server
...
accents; fhir-oracle shipped `ILIKE`, which ignores only case, so `family=muller`
...
too low" was "rebuild it". `--pool-size` beats `FHIR_ORACLE_POOL_SIZE`, so a
...
`fhir_oracle_request_latency_micros_total`, which with a request count gives only
...
`fhir_oracle_request_latency_seconds` is now a Prometheus histogram over the
...
- Pool size is configurable via `FHIR_ORACLE_POOL_SIZE` instead of compiled in.
- `crates/fhir-oracle-store/tests/concurrency.rs`: torn reads, racing conditional
  creates, and racing `If-Match` updates (T11.6).

**Added — the audit trail (M3.15–M3.17, §12)**

fhir-oracle still does not authenticate; the perimeter does. But "authentication
...
- Every **read** appends a disclosure record to `fhir_oracle_access_log`: read,
...
- History is **tamper-evident**: a per-resource SHA-256 chain
  ...
  `now()` and cannot race the read of the previous hash. `fhir-oracle
...
The restriction was never fhir-oracle's: the published `fhir` 1.2.0 could not
...
fhir-oracle's floor moves to `fhir = "1.2.1"` with `features = ["r3", "r4"]`, and
...
- The CapabilityStatement now declares `versioning`, `readHistory`,
  ...
  `security` block stating plainly that fhir-oracle verifies no identity (A7.12).
- CI gains a supply-chain job (cargo-deny, CycloneDX SBOM) and `deny.toml`
  (O10.10).
- `book/src/trust-boundary.md`: what fhir-oracle guarantees, what the deployment
...
> CLI in the bullets below never existed in `fhir-oracle` (`C0.17`, `C0.18`,
...
- FHIR REST server (`fhir-oracle serve`): every installed version mounted,
