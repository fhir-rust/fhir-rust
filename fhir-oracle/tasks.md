...
> against MySQL 8.4 — an engine this port never provisioned. `crates/fhir-oracle-store/`
> contains `lib.rs` and `chain.rs`, has no `tests/` directory, and depends on no
> database driver. This port is **Scaffold**.
>
> Its DDL emitter is also still verbatim MySQL
> ([F-08](../spec/audit.md#f-08)), so it cannot yet produce an Oracle schema at
> all.
...
- [x] **T1 Workspace scaffold.** Cargo workspace per plan D14
  (`fhir-oracle-map`, `fhir-oracle-gen`, `fhir-oracle-store`, `fhir-oracle` — the server crate
...
- [x] **T3 Relational map format.** `fhir-oracle-map::model`: node arena (cycles
  ...
  abbreviation + hash fallback. Assets: `assets/fhir-oracle-relmap-{r3,r4,r5}
...
- [x] **T9 Round-trip property tests.** Map-driven random-resource
  ...
  (`FHIR_ORACLE_PROPTEST_CASES`; default 500 locally).
...
- [x] **T12 fhir_oracle_meta + idempotent init.** Staged-schema install +
  atomic rename, checksum recorded; re-init no-ops on matching checksum
  and refuses a mismatch. Chunked `drop_schema` + `fhir-oracle drop --yes`.
...
- [~] **T14 Query builder + result params.** Done: `Store::search` — AND
  ...
  unsupported-parameter errors; `fhir-oracle search` CLI; `_sort` (base-table
...
- [x] **T16 axum skeleton.** `fhir-oracle-server` crate + `fhir-oracle serve`:
...
> README claims all three corpora round-trip losslessly, and `fhir-oracle serve`
...
- [ ] **T22 R3 artifacts.** Same for 3.0.2.
  *Accept:* full R3 examples corpus round-trips live; REST suite green on
  `/r3`.
- [x] **T23 Multi-version serve.** `fhir-oracle serve` mounts every version
...
- [x] **T-validate (V9.2).** `fhir-oracle load --validate` deserializes each
...
- [x] **T-graceful.** `fhir-oracle serve` shuts down cleanly on SIGINT/SIGTERM.
...
- [~] **T25 Pool + timeout hardening.** Done: server-side
  statement_timeout (FHIR_ORACLE_STATEMENT_TIMEOUT_MS, default 30 s), pool
...
- [x] **T26 Migrations + upgrade.** `init` stores the map asset in
  fhir_oracle_meta; `init --upgrade` diffs installed vs current maps and
...
- [~] **T30 Security review + release.** Done: LICENSE-MIT/APACHE,
  ...
  assets embedded in the binary so `cargo install fhir-oracle` is
...
- [x] **T31 Snapshot reads (R4.5).** `Store::get` reads the base row and
  ...
  (`fhir-oracle-store/src/lib.rs:554`), while `put_in` deletes and re-inserts
...
- [x] **T32 Encrypted database transport (O10.7).** `NoTls` is hard-coded
  (`fhir-oracle-store/src/lib.rs:186`), so PHI crosses to PostgreSQL in clear and
  ...
  *Done:* `SslPolicy`, rustls connector, `PGSSLROOTCERT` trust anchors,
  ...
  whenever the link is unencrypted. fhir-oracle's `require` validates the
...
- [x] **T33 Atomic conditional interactions (A7.10).** `If-None-Exist`
  searches then writes (`fhir-oracle-server/src/lib.rs:444`); two concurrent
...
- [x] **T35 Configured service base URL (A7.7).** Bundle `fullUrl` and
  ...
  (`fhir-oracle-server/src/lib.rs:739`). Add `--base-url`, honor forwarded
...
- [x] **T38 Resource id validation (R4.6).** No `[A-Za-z0-9\-\.]{1,64}`
  ...
  *Done:* `valid_fhir_id` guards read, vread, history, update, delete, and
  Bundle entry urls. *Remaining:* the same check inside `fhir-oracle load`.
...
- [x] **T41 Access log (PR12.5, PR12.6).** `fhir_oracle_access_log` per schema;
...
- [x] **T42 Tamper-evident history (M3.16, M3.17).** `prev_hash`/`row_hash`
  ...
  `fhir-oracle verify-audit` walking every chain, and the documented `REVOKE`
...
- [x] **T59 Tamper evidence that survives the database (M3.16a-c).** Two
  ...
  *A defect this work introduced.* The rotation test set `FHIR_ORACLE_CHAIN_KEY`
...
- [x] **T43 Worldwide string search (P6.6).** Each string search target
  ...
  *Two traps, both found by measurement rather than reasoning.*
  ...
  First: the cheap-looking version — an expression index on `fhir_oracle_norm(col)`
  plus a predicate `fhir_oracle_norm(col) LIKE fhir_oracle_norm($1) || '%'` — **does not
  use the index**. PostgreSQL's btree prefix optimization for `LIKE` needs a
  *constant* pattern, and `fhir_oracle_norm($1)` is only stable, not constant.
  ...
  lives in Rust only (`fhir-oracle-map::fold`), and the database stores its output.
...
- [x] **T44 Edge resource limits (O10.8, P6.7).** Per-request timeout,
  ...
  `--pool-size` takes precedence over `FHIR_ORACLE_POOL_SIZE`: a flag the operator
...
- [x] **T45 Admin plane separation (O10.9).** `--admin-bind` for
  ...
  histogram (`fhir_oracle_request_latency_seconds`, default 1ms–10s buckets) so
...
- [x] **T46 Honest CapabilityStatement (A7.12).** Declare
  ...
  `security.description` stating plainly that fhir-oracle verifies no identity.
...
- [x] **T48 Concurrency, redaction, and audit test suites (T11.6–T11.8).**
  ...
  than dropping.
- [x] **T49 Erasure (M3.18).** `fhir-oracle purge` with tombstone rows,
  ...
  `fhir-oracle purge`, the tombstone carrying who/when/why plus the terminated
  chain hash, and the append-only trigger relaxed to permit `DELETE` only
  inside a transaction that sets `fhir_oracle.erasure`. The book states the two
...
- [x] **T50 Trust-boundary chapter (PR12.8).** One table in the book: what
  fhir-oracle guarantees, what the perimeter must provide, what neither does yet.
...
- [ ] **T52 Bulk Data `$export`.** Async kickoff, NDJSON output, status
  polling — ONC/HTI expects it, and the earlier fhir-oracle had the client half.
...
- [ ] **T57 Restore and failover drills.** A documented, tested PITR
  restore and a `fhir-oracle fsck` that checks orphan rows, ordinal gaps, and
...
- [x] **T60 Rename repair.** The initial `fhirpg` → `fhir-oracle` substitution
...
- [x] **T63 DDL for MySQL.** `ddl.rs` re-emitted: backquoted identifiers;
  ...
  per-table `SIGNAL` triggers gated on `@fhir_oracle_erasure`; the `_norm`
...
- [x] **T70 Divergence policy (M14.0a–M14.0g).** fhir-oracle and fhir-mariadb are
...
- [x] **T72 CI retargeted to the real engine.** The inherited pipelines still
  ...
  The `mysql_ddl` suite runs with `FHIR_ORACLE_DDL_FULL=1`, installing every
...
- [x] **T73 Reduced to an embeddable library.** Scope correction: this project
  ...
  `crates/fhir-oracle/assets/` moved to a top-level `assets/`, with test paths
...
- [~] **T74 Native MySQL store — started.** Same parallel-module approach the
  ...
  *Also landed:* the access log and `purge` (GDPR Art. 17, M3.18). The
  append-only trigger permits the delete only while `@fhir_oracle_erasure` is
...
- [x] **T75 PostgreSQL store removed from fhir-oracle.** The inherited `Store` is
  gone, and with it `tokio-postgres`, `deadpool-postgres`,
  `tokio-postgres-rustls`, `rustls`, `rustls-native-certs`, and `futures-util`.
  `fhir-oracle-store` now depends on `mysql_async`, `serde_json`, `tokio`,
...
- [ ] **T80 Scaffold only — the DDL is NOT ported.** Forked from `fhir-mysql`
  for its structure. The MySQL store and search module were deleted rather than
  renamed, but `ddl.rs` is **still the MySQL emitter**, and its unit tests still
  assert MySQL behaviour. Those tests are `#[ignore]`d with that reason attached,
  so a green run cannot be mistaken for Oracle conformance — 11 of them.
  *Why none of the remaining work is mechanical:*
  - `VARCHAR2(n CHAR)`/`CLOB`/`NUMBER`/`TIMESTAMP(6)`; no boolean column type
    before 23ai, so `Bool` becomes `NUMBER(1)` with a CHECK.
  - `VARCHAR2` maxes at 4000 bytes (32767 only with extended types, a
    database-level setting a schema cannot assume). Longer values are `CLOB`, and
    a `CLOB` cannot be indexed or compared like text — a sharper form of the
    constraint the SQL Server port hit.
  - No `IF NOT EXISTS` anywhere: idempotence means wrapping every statement in a
    PL/SQL block that swallows ORA-00955.
  - Identifiers were 30 bytes before 12.2 and 128 after, so the port must declare
    a version floor rather than inherit `PG_MAX_IDENT = 63` silently.
  - Triggers use `RAISE_APPLICATION_ERROR`, and the erasure flag needs an
    application context via `SYS_CONTEXT` — there is no session variable.
  *Also unresolved:* no Oracle driver is chosen yet, and Oracle Database Free
  container images need checking for arm64 before live verification is possible.

## Remaining work (as of the port commit)

Scaffold only. `ddl.rs` is still the MySQL emitter, and its eleven
MySQL-asserting tests are `#[ignore]`d so a green run cannot be mistaken for
Oracle conformance. Nothing here is an Oracle schema.

- [ ] **Port `ddl.rs` to Oracle.** The module header lists what changes and why
  none of it is mechanical: `VARCHAR2(n CHAR)`/`CLOB`/`NUMBER`/`TIMESTAMP(6)`;
  no boolean type before 23ai; `VARCHAR2` capped at 4000 bytes with longer values
  becoming `CLOB`, which cannot be indexed or compared like text; no
  `IF NOT EXISTS` anywhere, so idempotence means a PL/SQL block swallowing
  ORA-00955; `RAISE_APPLICATION_ERROR` for triggers; `SYS_CONTEXT` with an
  application context for the erasure flag.
- [ ] **Declare a version floor.** Identifiers were 30 bytes before 12.2 and 128
  after, so `PG_MAX_IDENT = 63` is only safe on 12.2+ and the port must say so
  rather than inherit the assumption.
- [ ] **Choose a driver**, and check whether an Oracle Database Free image runs
  on arm64 before promising live verification.
- [ ] **Then the store**, following the pattern the other ports established.
