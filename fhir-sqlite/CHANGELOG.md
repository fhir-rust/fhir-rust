...
`fhir-sqlite init --upgrade` for the new digest, tag, and counter-signature
columns; existing rows keep verifying, with the new chains reported as
beginning where their first digest appears.


**Changed — hash chains are computed by fhir-sqlite, not by PostgreSQL.** The
...
`FHIR_SQLITE_CHAIN_KEY` and never written to the database or logged. It sits
...
additive: retired keys stay loadable via `FHIR_SQLITE_CHAIN_KEYS_RETIRED` and
...
**Added — `fhir-sqlite chain-witness` and automatic checkpoints.** A tag proves a
...
fhir-sqlite cannot enforce that and does not claim to.
...
version of fhir-sqlite that had already shipped.
...
without `--allow-insecure-db`. fhir-sqlite's `require` validates the server
...
accents; fhir-sqlite shipped `ILIKE`, which ignores only case, so `family=muller`
...
too low" was "rebuild it". `--pool-size` beats `FHIR_SQLITE_POOL_SIZE`, so a
...
`fhir_sqlite_request_latency_micros_total`, which with a request count gives only
...
`fhir_sqlite_request_latency_seconds` is now a Prometheus histogram over the
...
- Pool size is configurable via `FHIR_SQLITE_POOL_SIZE` instead of compiled in.
- `crates/fhir-sqlite-store/tests/concurrency.rs`: torn reads, racing conditional
  creates, and racing `If-Match` updates (T11.6).

**Added — the audit trail (M3.15–M3.17, §12)**

fhir-sqlite still does not authenticate; the perimeter does. But "authentication
...
- Every **read** appends a disclosure record to `fhir_sqlite_access_log`: read,
...
- History is **tamper-evident**: a per-resource SHA-256 chain
  ...
  `now()` and cannot race the read of the previous hash. `fhir-sqlite
...
The restriction was never fhir-sqlite's: the published `fhir` 1.2.0 could not
...
fhir-sqlite's floor moves to `fhir = "1.2.1"` with `features = ["r3", "r4"]`, and
...
- The CapabilityStatement now declares `versioning`, `readHistory`,
  ...
  `security` block stating plainly that fhir-sqlite verifies no identity (A7.12).
- CI gains a supply-chain job (cargo-deny, CycloneDX SBOM) and `deny.toml`
  (O10.10).
- `book/src/trust-boundary.md`: what fhir-sqlite guarantees, what the deployment
...
- FHIR REST server (`fhir-sqlite serve`): every installed version mounted,
