...
| Host port | `19999` |
| Container | `fhir-sqlite-db` |

SQLite has no server, so the container exists for a different reason: it holds a
**pinned** `sqlite3`. SQLite's DDL features move between releases, so checking
the generated schema against whatever version the laptop happens to ship is a
weaker claim than checking it against a known one. `up` installs sqlite3 in the
container and writes `target/sqlite3-container`, a wrapper the tests invoke via
`FHIR_SQLITE_SQLITE3`; the repo is mounted at the same path inside the
container, which is why the tests keep scratch databases under `target/` rather
than in `TMPDIR`. With the variable unset the tests use the host `sqlite3`.
...
cargo test -p fhir-sqlite-store --test live -- --nocapture --test-threads=1
```
...
Set `FHIR_SQLITE_SPEC_DIR` to point somewhere else. The tests themselves fall back to
...
| Variable | Meaning |
|---|---|
| `FHIR_SQLITE_SQLITE3` | Set by `dsn`. Without it the live tests skip. |
...
**Port already in use** — something else is on `19999`, often a
