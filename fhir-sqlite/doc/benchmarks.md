...
- Install time for full R5 (7,355 tables + 9,168 indexes, of which 1,813 are
  ...
  and all: this port installs via a direct statement-by-statement install —
  SQLite has no transactional DDL and no schema-rename, so a failed install is
  cleaned up by unlinking the file, since the schema **is** the file
  (corrected under **F-27**). `max_locks_per_transaction` is a PostgreSQL
  setting this port's engine does not have.
...
- **No live corpus round-trip benchmark exists for this port.** The 101s/13ms
  ...
  `fhir-sqlite` has no equivalent harness — its live suite tests correctness
...
`FHIR_SQLITE_BENCH`. `cargo test --release -p fhir-sqlite-store --test bench`
