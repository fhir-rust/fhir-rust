...
fuzz_target!(|data: &[u8]| {
    ...
    let sort: Vec<fhir_sqlite_store::sqlite_search::SortKey> = params
        .iter()
        .take(2)
        .map(|(k, _)| fhir_sqlite_store::sqlite_search::SortKey {
            code: k.clone(),
            descending: k.starts_with('-'),
        })
        .collect();

    let Ok(query) = fhir_sqlite_store::sqlite_search::build_search_sql(
        map,
        rm,
        &params,
        50,
    ...
    let replaced: Vec<(String, String)> = params
        ...
        .collect();
    if let Ok(shadow) = fhir_sqlite_store::sqlite_search::build_search_sql(
        map,
        rm,
        &replaced,
        50,
