...
// SQLite needs no server, and the map is compiled in.
let map = Arc::new(fhir_sqlite_map::RelMap::bundled("r5")?);   // compiled in
...
all core reads and writes, `conditional_create`/`conditional_delete`, `emit_checkpoint`, erasure and audit verification.

`chain_witness` and `resign_history` are absent; `transact_audited` carries a stated limitation — see the [conformance matrix](https://github.com/fhir-rust/fhir-rust/blob/main/spec/databases/conformance-matrix.md) for the operation-by-operation table.
...
112 tests, none needing a server — which is also the caveat: most guarantees here are database guarantees.
