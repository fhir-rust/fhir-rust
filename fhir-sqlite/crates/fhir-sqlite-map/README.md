...
```rust
use std::sync::Arc;
use fhir_sqlite_map::{RelMap, shred, reconstruct};
...
| `ddl` | **the only dialect-specific file** — emits SQLite DDL |
...
| Column | Purpose |
| --- | --- |
| `<col>_norm` | the folded form, so prefix search is case- and accent-insensitive (`TEXT COLLATE BINARY`) |
