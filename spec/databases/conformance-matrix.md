...
| `M3.17` append-only trigger | • | • | • | • | • | ~ | oracle: a forbidden `UPDATE` and `DELETE` were each refused on a live 26ai history table and a declared erasure then succeeded (`M14.29`) — verified by hand, and now also exercised live through the store's own erasure-escape path (`purge_erases_history_and_leaves_a_verifiable_hole`); still `~` not `•` because no test drives a store-mediated *forbidden* delete and checks it is refused — **confirmed absent, not assumed, when F-51 closed** (2026-08-29): neither `M14.29` nor `M14.8` appears anywhere in `fhir-oracle-store`'s live suite. mssql: live-verified — a raw `DELETE` on history is refused without the `SESSION_CONTEXT` erasure flag, `purge` succeeds with it |
