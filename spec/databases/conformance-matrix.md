...
| `T11.2` live gate | • | • | • | • | • | • | mssql: 33 of 33 store-crate tests green against live `azure-sql-edge`, 0 `#[ignore]`d, plus the DDL test (an earlier revision said 23 — the pre-`upgrade.rs`/`ssl_live.rs` count, F-74). oracle: 20 of 20 store-crate tests (7 `oracle_store.rs`: `init`/`put`-`get`/rewrite/`history`-`vread`-`delete`-`verify_audit`/`purge`/`search`/`log_access`; 12 `upgrade.rs`; 1 `root_extension.rs`) green against live `gvenzl/oracle-free:23-slim-faststart`, 0 ignored, run `--test-threads=1` (**F-68**, **F-15**, **F-85**) |
