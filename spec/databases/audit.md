...
| [F-67](#f-67) | **High** | Four TLS advisories `deny.toml` excused as dev-dependency-only now reach the shipping `fhir-mssql-store`; `native-tls` fails the handshake on this host | **open** — a driver-replacement investigation (2026-08-28, `M14.34` in `fhir-mssql/spec/14-mssql-dialect.md`) found no viable near-term fix: the one upstream PR that fixes it (`prisma/tiberius#419`) has no maintainer review in 3+ months and cannot be used by a published crate anyway (cargo forbids git dependencies in one), and the one new alternative crate (`ms-tds`) disqualifies itself by advertising offensive/exploitation tooling alongside its driver code. Still awaiting the owner's decision between the two options that remain: accept formally, or mark the TLS story unresolved |
