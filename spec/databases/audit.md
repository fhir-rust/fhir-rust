...
| [F-67](#f-67) | **High** | Four TLS advisories `deny.toml` excused as dev-dependency-only now reach the shipping `fhir-mssql-store`; `native-tls` fails the handshake on this host | **open — accepted.** Owner decision 2026-08-28: keep shipping on upstream `tiberius`, document the risk loudly, do not chase a replacement. Reached after investigating three alternatives (`M14.34` in `fhir-mssql/spec/14-mssql-dialect.md`): a from-scratch driver (~3–4.5 months, and worse on the trust axis than the flawed incumbent); the upstream PR that already fixes it (`prisma/tiberius#419` — clean and CI-green on its own fork, but no maintainer review in 3+ months, and unusable by a published crate regardless since cargo forbids a `git` dependency in one); a fork carrying that same patch (1–2 weeks private / 2–3 weeks published under our own name, plus an *open-ended* tail — every future advisory in `tiberius`'s tree becomes ours alone to patch); and one newer alternative crate, `ms-tds`, which disqualifies itself by advertising offensive/exploitation tooling alongside its driver code. The advisories stay technically unpatched — this row stays **open** — but the decision about them is made, not deferred |
