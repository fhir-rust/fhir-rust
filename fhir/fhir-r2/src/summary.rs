//! Summary serialization for FHIR R2 (the `_summary=true` view).
//!
...
//! [`meta`](crate::r2::meta) table.
//!
//! The pruning itself does not vary by release and lives in [`::fhir_core::summary`];
//! this module binds it to the R2 element table.
//!
//! ```
//! use fhir::r2::resources::Patient;
//! use fhir::r2::summary::to_summary_value;
//!
...
mod tests {
    ...
    fn patient_summary_keeps_summary_drops_the_rest() {
        let patient: Patient = ::serde_json::from_value(::serde_json::json!({
            ...
            // DSTU2 `HumanName.family` repeats (0..*); R3 made it a single
            // string. This is the kind of difference a shared model type
            // would have to paper over, and why the releases stay separate.
