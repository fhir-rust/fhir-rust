//! A R3 (3.0.2) resource that parses must serialize and re-parse identically.
//!
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(v) = serde_json::from_str::<serde_json::Value>(text) {
        let _ = serde_json::from_value::<fhir::r3::resources::Resource>(v);
    }
    let Ok(resource) = serde_json::from_str::<fhir::r3::resources::Resource>(text) else {
        return;
    };
    let once = serde_json::to_value(&resource).expect("a parsed resource must serialize");
    let again: fhir::r3::resources::Resource =
