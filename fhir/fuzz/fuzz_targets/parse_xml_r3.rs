//! Adversarial XML must not crash the R3 (3.0.2) reader (spec R13.17).
//!
//! The reader is shared across releases but is driven by `r3`'s own
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(value) = fhir::xml::xml_to_value(fhir::r3::meta::elements(), text) {
