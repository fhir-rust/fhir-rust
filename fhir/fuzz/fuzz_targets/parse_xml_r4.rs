//! Adversarial XML must not crash the R4 (4.0.1) reader (spec R13.17).
//!
//! The reader is shared across releases but is driven by `r4`'s own
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(value) = fhir::xml::xml_to_value(fhir::r4::meta::elements(), text) {
