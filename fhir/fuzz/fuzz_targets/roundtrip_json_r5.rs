//! A R5 (5.0.0) resource that parses must serialize and re-parse identically.
//!
...
//! or a rename that round-trips in R5 says nothing about r5.
#![no_main]
#![forbid(unsafe_code)]
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(v) = serde_json::from_str::<serde_json::Value>(text) {
        let _ = serde_json::from_value::<fhir::r5::resources::Resource>(v);
    }
    let Ok(resource) = serde_json::from_str::<fhir::r5::resources::Resource>(text) else {
        return;
    };
    let once = serde_json::to_value(&resource).expect("a parsed resource must serialize");
    let again: fhir::r5::resources::Resource =
