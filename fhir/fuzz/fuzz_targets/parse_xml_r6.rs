//! Adversarial XML must not crash the R6 (6.0.0-ballot3) reader (spec R13.17).
//!
//! The reader is shared across releases but is driven by `r6`'s own
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(value) = fhir::xml::xml_to_value(fhir::r6::meta::elements(), text) {
