//! Element metadata extracted from the FHIR R6 specification.
//!
...
//! it — regenerate with `cargo run -- r6`.
//!
...
/// Look up the metadata for an R6 element by its full FHIR path.
///
...
/// Every extracted R6 element, sorted by path.
#[must_use]
...
mod tests {
    ...
    // Acceptance fact 2: Observation.value[x] is a choice with the R6 type set.
    // (The task text said 11 types from R4; R6 adds Attachment and Reference,
    // for 13.)
    #[test]
    fn observation_value_choice_types() {
        let value = element("Observation.value[x]").expect("Observation.value[x]");
        assert!(value.is_choice());
        assert_eq!(value.types.len(), 13, "R6 Observation.value[x] type count");
        for code in [
            "Quantity",
            "CodeableConcept",
            "string",
