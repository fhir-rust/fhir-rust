//! Reserved for a future FHIR Release 8 data model.
//!
//! **There is no FHIR R8 specification.** At the time of writing, R6 is in
//! ballot (6.0.0-ballot3) and is the newest release HL7 has published in any
//! form. This crate holds the name so that the model can be published here
...
//!    `doc/fhir-specifications/r8/fhir-definitions-json/`.
//! 2. Add `Version::R8` to `src/codegen/version.rs` (module, label, version
//!    string, spec URL, parse tokens).
//! 3. Add `"r8"` to `KNOWN_VERSIONS` in `fhir-derive-macros`.
//! 4. Run `cargo run -- r8`, which writes the model into `fhir-release-8/src`.
