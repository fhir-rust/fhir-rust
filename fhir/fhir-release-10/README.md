...
published in any form, and it is still in ballot. R10 is four numbers past
anything that exists; HL7 has not announced it, and nothing here should be
...
`fhir-release-9` and `fhir-release-10` sort adjacently by number but not as
strings: a tool ordering the family lexically will place `-10` before `-2`.
Nothing here depends on that ordering, but anyone scripting over the family
