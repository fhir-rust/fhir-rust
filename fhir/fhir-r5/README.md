...
The FHIR **Release 5 (5.0.0)** data model: every resource and datatype as typed
Rust, with validation, builders, and serde.

158 resources, 50 datatypes, 21 primitives, 442 code enums.
...
```toml
fhir = { version = "2", default-features = false, features = ["r5"] }
```

The facade re-exports this crate as `fhir::r5`, which is the path to
write in your code. Which crate provides it is an implementation detail.

## Status

R5 is the current normative release, published in 2023. It is the default
feature of the `fhir` facade: depending on `fhir` with no feature selection
gives you this crate and nothing else.

## How this release differs

| | R5 | R4 |
| --- | --- | --- |
| `Observation.value[x]` | 13 types | 11 |
| Medication references | `CodeableReference` | `medication[x]` choice |
| `integer64` | present | absent |
| `RatioRange`, `CodeableReference` | present | absent |

`CodeableReference` is the change most likely to break a port from R4: what
was a choice between a code and a reference became a single type holding
either or both.
...
use fhir::r5::resources::{Observation, Resource};
use fhir::r5::types;
use fhir::validate::Validate;
