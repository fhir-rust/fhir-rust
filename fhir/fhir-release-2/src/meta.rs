//! Element metadata extracted from the FHIR R2 specification.
//!
...
//! it — regenerate with `cargo run -- r2`.
//!
...
/// Look up the metadata for an R2 element by its full FHIR path.
///
...
/// Every extracted R2 element, sorted by path.
#[must_use]
...
mod tests {
    ...
    // `Observation.value[x]` allows 11 types in R2 and 11 in R4 — but not the
    // same 11. R2 permits `Attachment` and not `integer`; R4 reversed both, and
    ...
    fn observation_value_choice_types() {
        ...
        assert!(value.is_choice());
        // DSTU2 Observation.value[x] has ten types; R3 added Boolean.
        assert_eq!(value.types.len(), 10, "R2 Observation.value[x] type count");
        for code in [
            "Quantity",
            "CodeableConcept",
            "string",
        ...
        assert!(
            !value.type_codes().any(|c| c == "integer"),
            "integer arrived in R4"
        );
        assert!(
            !value.type_codes().any(|c| c == "Reference"),
            "Reference arrived in R5"
        );
    }
