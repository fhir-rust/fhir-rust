...
mssql)
  ...
  # `tiberius` probe, generated into `target/` the same way the sqlite branch
  # generates its `$WRAPPER` — not a workspace member, so it does not become a
  # second CLI crate (`C0.17`/`C0.18`) — and pinned to the same driver version
  # this port's own store depends on.
