...
mssql)
  ...
  # A real query, not a port check. SQL Server accepts TCP well before it has
  # recovered the system databases, and a CREATE SCHEMA issued in that window
  # fails in a way that looks like a DDL bug.
  ready() {
    ct exec "$NAME" sh -c \
  ...
  dsn_line() {
    echo "export ${ENV_VAR}='server=tcp:127.0.0.1,${PORT};user=sa;password=${SA_PASSWORD};TrustServerCertificate=true'"
  }
  client_cmd() {
    ct exec -it "$NAME" sh -c \
