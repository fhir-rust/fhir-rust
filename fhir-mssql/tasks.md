...
> trust.
>
> This port is worse: it is `fhir-mysql`'s file with the crate name substituted,
> so the **store** milestones are checked off too, citing acceptance runs
> against MySQL 8.4 — an engine this port never provisioned. `crates/fhir-mssql-store/`
> contains `lib.rs` and `chain.rs`, has no `tests/` directory, and depends on no
> database driver. This port is **Scaffold**.
...
- [x] **T1 Workspace scaffold.** Cargo workspace per plan D14
  (`fhir-mssql-map`, `fhir-mssql-gen`, `fhir-mssql-store`, `fhir-mssql` — the server crate
...
- [x] **T3 Relational map format.** `fhir-mssql-map::model`: node arena (cycles
  ...
  abbreviation + hash fallback. Assets: `assets/fhir-mssql-relmap-{r3,r4,r5}
...
- [x] **T9 Round-trip property tests.** Map-driven random-resource
  ...
  (`FHIR_MSSQL_PROPTEST_CASES`; default 500 locally).
...
- [x] **T12 fhir_mssql_meta + idempotent init.** Staged-schema install +
  atomic rename, checksum recorded; re-init no-ops on matching checksum
  and refuses a mismatch. Chunked `drop_schema` + `fhir-mssql drop --yes`.
...
- [~] **T14 Query builder + result params.** Done: `Store::search` — AND
  ...
  unsupported-parameter errors; `fhir-mssql search` CLI; `_sort` (base-table
...
- [x] **T16 axum skeleton.** `fhir-mssql-server` crate + `fhir-mssql serve`:
...
> README claims all three corpora round-trip losslessly, and `fhir-mssql serve`
...
- [ ] **T22 R3 artifacts.** Same for 3.0.2.
  *Accept:* full R3 examples corpus round-trips live; REST suite green on
  `/r3`.
- [x] **T23 Multi-version serve.** `fhir-mssql serve` mounts every version
...
- [x] **T-validate (V9.2).** `fhir-mssql load --validate` deserializes each
...
- [x] **T-graceful.** `fhir-mssql serve` shuts down cleanly on SIGINT/SIGTERM.
...
- [~] **T25 Pool + timeout hardening.** Done: server-side
  statement_timeout (FHIR_MSSQL_STATEMENT_TIMEOUT_MS, default 30 s), pool
...
- [x] **T26 Migrations + upgrade.** `init` stores the map asset in
  fhir_mssql_meta; `init --upgrade` diffs installed vs current maps and
...
- [~] **T30 Security review + release.** Done: LICENSE-MIT/APACHE,
  ...
  assets embedded in the binary so `cargo install fhir-mssql` is
...
- [x] **T31 Snapshot reads (R4.5).** `Store::get` reads the base row and
  ...
  (`fhir-mssql-store/src/lib.rs:554`), while `put_in` deletes and re-inserts
...
- [x] **T32 Encrypted database transport (O10.7).** `NoTls` is hard-coded
  (`fhir-mssql-store/src/lib.rs:186`), so PHI crosses to PostgreSQL in clear and
  ...
  *Done:* `SslPolicy`, rustls connector, `PGSSLROOTCERT` trust anchors,
  ...
  whenever the link is unencrypted. fhir-mssql's `require` validates the
...
- [x] **T33 Atomic conditional interactions (A7.10).** `If-None-Exist`
  searches then writes (`fhir-mssql-server/src/lib.rs:444`); two concurrent
...
- [x] **T35 Configured service base URL (A7.7).** Bundle `fullUrl` and
  ...
  (`fhir-mssql-server/src/lib.rs:739`). Add `--base-url`, honor forwarded
...
- [x] **T38 Resource id validation (R4.6).** No `[A-Za-z0-9\-\.]{1,64}`
  ...
  *Done:* `valid_fhir_id` guards read, vread, history, update, delete, and
  Bundle entry urls. *Remaining:* the same check inside `fhir-mssql load`.
...
- [x] **T41 Access log (PR12.5, PR12.6).** `fhir_mssql_access_log` per schema;
...
- [x] **T42 Tamper-evident history (M3.16, M3.17).** `prev_hash`/`row_hash`
  ...
  `fhir-mssql verify-audit` walking every chain, and the documented `REVOKE`
...
- [x] **T59 Tamper evidence that survives the database (M3.16a-c).** Two
  ...
  *A defect this work introduced.* The rotation test set `FHIR_MSSQL_CHAIN_KEY`
...
- [x] **T43 Worldwide string search (P6.6).** Each string search target
  ...
  *Two traps, both found by measurement rather than reasoning.*
  ...
  First: the cheap-looking version — an expression index on `fhir_mssql_norm(col)`
  plus a predicate `fhir_mssql_norm(col) LIKE fhir_mssql_norm($1) || '%'` — **does not
  use the index**. PostgreSQL's btree prefix optimization for `LIKE` needs a
  *constant* pattern, and `fhir_mssql_norm($1)` is only stable, not constant.
  ...
  lives in Rust only (`fhir-mssql-map::fold`), and the database stores its output.
...
- [x] **T44 Edge resource limits (O10.8, P6.7).** Per-request timeout,
  ...
  `--pool-size` takes precedence over `FHIR_MSSQL_POOL_SIZE`: a flag the operator
...
- [x] **T45 Admin plane separation (O10.9).** `--admin-bind` for
  ...
  histogram (`fhir_mssql_request_latency_seconds`, default 1ms–10s buckets) so
...
- [x] **T46 Honest CapabilityStatement (A7.12).** Declare
  ...
  `security.description` stating plainly that fhir-mssql verifies no identity.
...
- [x] **T48 Concurrency, redaction, and audit test suites (T11.6–T11.8).**
  ...
  than dropping.
- [x] **T49 Erasure (M3.18).** `fhir-mssql purge` with tombstone rows,
  ...
  `fhir-mssql purge`, the tombstone carrying who/when/why plus the terminated
  chain hash, and the append-only trigger relaxed to permit `DELETE` only
  inside a transaction that sets `fhir_mssql.erasure`. The book states the two
...
- [x] **T50 Trust-boundary chapter (PR12.8).** One table in the book: what
  fhir-mssql guarantees, what the perimeter must provide, what neither does yet.
...
- [ ] **T52 Bulk Data `$export`.** Async kickoff, NDJSON output, status
  polling — ONC/HTI expects it, and the earlier fhir-mssql had the client half.
...
- [ ] **T57 Restore and failover drills.** A documented, tested PITR
  restore and a `fhir-mssql fsck` that checks orphan rows, ordinal gaps, and
...
- [x] **T60 Rename repair.** The initial `fhirpg` → `fhir-mssql` substitution
...
- [x] **T63 DDL for MySQL.** `ddl.rs` re-emitted: backquoted identifiers;
  ...
  per-table `SIGNAL` triggers gated on `@fhir_mssql_erasure`; the `_norm`
...
- [x] **T70 Divergence policy (M14.0a–M14.0g).** fhir-mssql and fhir-mariadb are
...
- [x] **T72 CI retargeted to the real engine.** The inherited pipelines still
  ...
  The `mysql_ddl` suite runs with `FHIR_MSSQL_DDL_FULL=1`, installing every
...
- [x] **T73 Reduced to an embeddable library.** Scope correction: this project
  ...
  `crates/fhir-mssql/assets/` moved to a top-level `assets/`, with test paths
...
- [~] **T74 Native MySQL store — started.** Same parallel-module approach the
  ...
  *Also landed:* the access log and `purge` (GDPR Art. 17, M3.18). The
  append-only trigger permits the delete only while `@fhir_mssql_erasure` is
...
- [x] **T75 PostgreSQL store removed from fhir-mssql.** The inherited `Store` is
  gone, and with it `tokio-postgres`, `deadpool-postgres`,
  `tokio-postgres-rustls`, `rustls`, `rustls-native-certs`, and `futures-util`.
  `fhir-mssql-store` now depends on `mysql_async`, `serde_json`, `tokio`,
...
- [~] **T80 Scaffold and T-SQL DDL — in progress.** Forked from `fhir-mysql`,
  which is the closest structural match (a server, bounded identifiers, hash
  surrogate keys). The MySQL store and search module were **deleted rather than
  renamed**: T-SQL is a different dialect, and code that says `mssql` while
  emitting MySQL is the worst kind of green.
  *Landed:* the type map (`BIT`, `NVARCHAR(MAX)`, `NVARCHAR(450) COLLATE
  Latin1_General_100_BIN2`, `DATETIME2(6)`), bracketed identifiers,
  `IF SCHEMA_ID(...) IS NULL EXEC(...)` for schema creation, `IDENTITY(1,1)`,
  and `CREATE OR ALTER TRIGGER` with `THROW 50000` — which makes each
  append-only guard one idempotent statement, closer to the PostgreSQL original
  than MySQL manages. The erasure flag reads `SESSION_CONTEXT`, T-SQL's nearest
  equivalent to `SET LOCAL`.
  *The constraint that shapes this port:* **SQL Server has no prefix indexes and
  cannot key on `NVARCHAR(MAX)` at all**, and its 900-byte key limit is tighter
  than MySQL's 3072. The MySQL port's prefix arithmetic is therefore gone. The
  folded companion column is `NVARCHAR(450)` — exactly 900 bytes — so every
  non-`:exact` string search still indexes; but a token's `system`/`code` are
  `NVARCHAR(MAX)` and are **dropped from their index**, which is a real
  performance gap recorded rather than hidden. Search stays correct, it scans.
  *Open decision:* the fix is a persisted computed column holding the leading 450
  characters, indexed in place of the original — indexable without truncating
  what is stored. That belongs in the generated map, not the DDL.
  *Live-verified.* The generated schema installs on a real SQL Server —
  131 statements, 102 tables, 4 triggers — via `tiberius` against
  `azure-sql-edge`, the build that runs on arm64. Full SQL Server images are
  amd64-only, and Azure SQL Edge is a subset, so this is good evidence rather
  than a conformance claim.
  *The DDL was written without a server to try it on, and running it found four
  real bugs the unit tests could not:*
  1. `NNVARCHAR(64)` — a `VARCHAR(64)` → `NVARCHAR(64)` substitution applied on
     top of a column that was already `NVARCHAR`.
  2. Twelve bare `TEXT` columns; the conversion pattern had only matched
     `TEXT NOT NULL`.
  3. `DATETIME(6)` and `CURRENT_TIMESTAMP(6)` left in the hand-written system
     tables — MySQL spellings with no T-SQL equivalent (`DATETIME2(6)` and
     `SYSUTCDATETIME()`), plus MySQL's `UNIQUE KEY`/`KEY` inline index syntax.
  4. The access log indexed `[actor](191)` — MySQL prefix syntax on a column that
     was `NVARCHAR(MAX)`, which SQL Server cannot index at all. `actor` is now
     `NVARCHAR(450)`, which is the 900-byte key limit and far beyond any real
     principal identifier.
  Every one of these compiled, and the shape-asserting unit tests passed
  throughout. This is the argument for verifying a dialect against its engine
  before building a store on top of it.
  *Remaining:* a `tiberius` store and the search builder (`OFFSET n ROWS FETCH
  NEXT m ROWS ONLY` rather than `LIMIT`).

## Remaining work (as of the port commit)

T-SQL DDL is ported and live-verified. There is no store at all yet.

- [ ] **A `tiberius` store.** `tiberius` is already a workspace dependency and
  proven against the server by the DDL test. Follow the parallel-module pattern
  the SQLite and MySQL ports used.
- [ ] **A search builder.** `OFFSET n ROWS FETCH NEXT m ROWS ONLY` rather than
  `LIMIT`; `@P1` placeholders; no `NULLS LAST`.
- [ ] **The unindexable-column decision.** A token's `system`/`code` are
  `NVARCHAR(MAX)` and are dropped from their index, so those searches scan. The
  fix is a persisted computed column holding the leading 450 characters, indexed
  in its place — indexable without truncating what is stored. It belongs in the
  generated map, not the DDL.
- [ ] **Verify against full SQL Server.** Everything so far is `azure-sql-edge`,
  which is the arm64 build and a subset of the product.
