...
//! Set FHIR_MSSQL_CORPUS_DIR to a directory containing r5/, r4/, stu3/
//! subdirectories of raw example JSON files.

use std::collections::HashMap;
use std::path::PathBuf;

use fhir_mssql_map::reconstruct::{InRow, ReconIn, reconstruct};
use fhir_mssql_map::shred::{SqlVal, shred};
use serde_json::Value;

/// Where the FHIR example corpus lives, if it has been fetched.
///
/// `FHIR_MSSQL_CORPUS_DIR` first, then `corpus/` beside the workspace root —
...
fn corpus_root() -> Option<PathBuf> {
    if let Ok(dir) = std::env::var("FHIR_MSSQL_CORPUS_DIR") {
...
///
/// `FHIR_MSSQL_SPEC_DIR` first, then `spec-cache/` beside the workspace root, to
...
fn spec_root() -> Option<PathBuf> {
    let candidates = [
        std::env::var("FHIR_MSSQL_SPEC_DIR").ok().map(PathBuf::from),
...
fn to_recon(rm: &fhir_mssql_map::ResourceMap, out: &fhir_mssql_map::ShredOut) -> ReconIn {
...
fn roundtrip_full_corpus() {
    ...
    for (cdir, sdir, schema) in [("stu3", "r3", "r3"), ("r4", "r4", "r4"), ("r5", "r5", "r5")] {
        ...
        let map = fhir_mssql_gen::generate(&defs, schema).expect("generate");
    ...
    assert!(
        ...
        if total == 0 {
            "the corpus directory exists but holds no stu3/r4/r5 examples: \
             the fetch did not happen. Fetch it, or unset FHIR_MSSQL_CORPUS_DIR \
             to skip this test."
                .to_string()
        } else {
