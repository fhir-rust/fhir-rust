//! The SQL Server store, exercised against a real server (T64).
//!
...
//! reason `doc/containers.md` warns that a live test finishing in 0.00s did
//! not run.
//!
//! Run with `--test-threads=1`. Every test installs and drops its own schema
//! against `azure-sql-edge`, and running them concurrently produced a live
//! deadlock (SQL Server error 1205) during schema teardown under the default
//! thread-per-core runner — heavy concurrent DDL against one small container,
//! not a bug in what any single test exercises.
...
///
/// Creating every table takes a long time — slow enough that the DDL suite
/// samples too. Named rather than counted: taking "the first six" is
/// alphabetical, so it silently excludes `Observation` and every test then
/// fails with `unknown resource type`, which looks like a store bug and is
/// not.
...
async fn fresh(schema: &str) -> Option<MsSqlStore> {
    ...
    let store = MsSqlStore::connect(&dsn()?, map).await.expect("connect");
...
async fn connect_fails_loudly_on_a_bad_dsn() {
    ...
    assert!(
        matches!(
            err,
            fhir_mssql_store::StoreError::Pool(_) | fhir_mssql_store::StoreError::Db(_)
        ),
...
async fn init_installs_tables_and_triggers() {
    let Some(store) = fresh("fhir_mssql_store_init").await else {
        eprintln!("skipping: set FHIR_MSSQL_TEST_DSN to run");
        return;
    };
    ...
    assert!(tables > 5, "expected a schema, got {tables} tables");
...
async fn a_failed_install_reports_how_far_it_got() {
    // Per-resource `CREATE TABLE` carries no `IF NOT EXISTS` guard (M14.17
    // notes T-SQL has none to spell one with beyond wrapping the statement,
    // which only the schema/meta system objects do) — `init` is documented as
    // a fresh install, so a second call over an existing schema fails at
    // whatever statement collides first, and has to say so rather than leave
    // an operator guessing whether the database is empty or half-built.
    let Some(store) = fresh("fhir_mssql_store_partial").await else {
        eprintln!("skipping: set FHIR_MSSQL_TEST_DSN to run");
        return;
    };
    ...
    assert!(
        msg.contains("statement") && msg.contains("of"),
        "the error should say how far the install got: {msg}"
    );

    // And the tables really are still there — the claim in the message is
    // true, and a failed second install did not tear down the first.
    assert_eq!(
...
/// `9.60` into an `f64` literal at compile time, so the trailing zero would be
/// gone before any store saw it. M3.6 is about preserving what was *parsed*.
...
async fn put_then_get_round_trips_a_resource() {
    let Some(store) = fresh("fhir_mssql_store_rt").await else {
        eprintln!("skipping: set FHIR_MSSQL_TEST_DSN to run");
        return;
    };
    ...
    // M3.6: 9.60 must not come back as 9.6. NVARCHAR(MAX) storage is what
    // makes this possible; DECIMAL would have padded it and FLOAT would have
    // dropped the trailing zero.
    assert_eq!(
...
async fn rewrite_replaces_children_and_bumps_the_version() {
    let Some(store) = fresh("fhir_mssql_store_rw").await else {
        eprintln!("skipping: set FHIR_MSSQL_TEST_DSN to run");
        return;
    };
    ...
    // One fewer note: the removed child row must be gone, not shadowed. That
    // is what `ON DELETE CASCADE` buys, and it only works because the foreign
    // keys are real.
...
async fn extensions_get_distinct_surrogate_keys() {
    // The surrogate is the primary key on Ext/Deep (M14.12). If two rows
    // hashed alike the second would be rejected as a duplicate, and the loss
    // would look like a shredding bug rather than a key collision — so this
    // checks that a resource with several extensions round-trips whole.
    let Some(store) = fresh("fhir_mssql_store_ext").await else {
        eprintln!("skipping: set FHIR_MSSQL_TEST_DSN to run");
        return;
    };
...
/// A throwaway key, deliberately constant: it protects nothing in a test, and
/// a generated one would make failures unreproducible.
fn test_keys(id: &str) -> fhir_mssql_store::chain::KeyRing {
    let k = fhir_mssql_store::chain::ChainKey::from_hex(
        id,
        "4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f",
    )
    .expect("key parses");
    fhir_mssql_store::chain::KeyRing::new(vec![k])
}

async fn fresh_keyed(schema: &str, keys: fhir_mssql_store::chain::KeyRing) -> Option<MsSqlStore> {
    let map = sampled("r5", schema, &["Observation", "Patient"])?;
    let store = MsSqlStore::connect(&dsn()?, map)
        .await
        .expect("connect")
...
async fn history_and_vread_distinguish_a_deletion() {
    let Some(store) = fresh("fhir_mssql_store_hist").await else {
        eprintln!("skipping: set FHIR_MSSQL_TEST_DSN to run");
        return;
    };
    ...
    // 'C' and 'U' are distinct, and the op is part of the hashed preimage, so
...
async fn verify_audit_accepts_a_clean_chain_and_catches_tampering() {
    let Some(store) = fresh_keyed("fhir_mssql_store_verify", test_keys("ci")).await else {
        eprintln!("skipping: set FHIR_MSSQL_TEST_DSN to run");
        return;
    };
    ...
    // tamperer has to drop it first — which is exactly the deliberate act
    // M3.17 is designed to make visible.
    store
        .exec_raw(
            "DROP TRIGGER [fhir_mssql_store_verify].[observation_history_append_only_upd_trg_ix]",
        )
        .await
        .expect("drop trigger");
    store
        .exec_raw(
            "UPDATE [fhir_mssql_store_verify].[observation_history] \
             SET [actor] = N'not-me' WHERE [version_id] = 1",
        )
        .await
        .expect("tamper");
...
async fn search_by_token_number_and_paging() {
    let Some(store) = fresh("fhir_mssql_store_search").await else {
        eprintln!("skipping: set FHIR_MSSQL_TEST_DSN to run");
        return;
    };
    ...
    // "9" > "10", so gt9 would miss both larger values without the FLOAT
    // cast (M14.8).
    let mut got = store
...
async fn search_values_are_bound_never_interpolated() {
    ...
    let q = fhir_mssql_store::mssql_search::build_search_sql(
        &map,
        rm,
    ...
    // And the emitted SQL is this port's own dialect, not the one it was
    // forked from.
    assert!(!q.sql.contains("ILIKE"), "ILIKE survived: {}", q.sql);
    assert!(
        !q.sql.contains('`'),
        "backquoted identifier survived: {}",
        q.sql
    );
    assert!(
        !q.sql.contains('"'),
    ...
    assert!(
        q.sql.contains('['),
        "identifiers are not bracketed: {}",
        q.sql
    );
    assert!(q.sql.contains("@P1"), "no numbered placeholder: {}", q.sql);
    assert!(
        !q.sql.contains('?'),
        "MySQL-style placeholder survived: {}",
        q.sql
    );
}
...
async fn purge_erases_history_and_leaves_a_verifiable_hole() {
    let Some(store) = fresh_keyed("fhir_mssql_store_purge", test_keys("ci")).await else {
        eprintln!("skipping: set FHIR_MSSQL_TEST_DSN to run");
        return;
    };
    ...
    // ...but the erasure left evidence that it happened. A hole you can see
    // is the point: silence would be indistinguishable from never existing.
    let h = store
        .history("Observation", "obs-1")
        .await
        .expect("history");
...
async fn history_cannot_be_deleted_without_the_erasure_flag() {
    // The session context value is what separates a sanctioned erasure from a
    // stray DELETE. It is per-connection (`SESSION_CONTEXT`, not `SET LOCAL`),
    // which is exactly why `purge` holds one connection for the whole
    // operation — a flag set on a pooled connection and a delete issued on
    // another would look like a broken trigger.
    let Some(store) = fresh("fhir_mssql_store_flag").await else {
        eprintln!("skipping: set FHIR_MSSQL_TEST_DSN to run");
        return;
    };
    ...
    let err = store
        .exec_raw("DELETE FROM [fhir_mssql_store_flag].[observation_history] WHERE [id] = N'obs-1'")
        .await
...
async fn disclosures_are_recorded() {
    let Some(store) = fresh("fhir_mssql_store_log").await else {
        eprintln!("skipping: set FHIR_MSSQL_TEST_DSN to run");
        return;
    };
    ...
    let rec = fhir_mssql_store::AccessRecord {
        audit: fhir_mssql_store::Audit {
            actor: "dr-who".into(),
            actor_source: Some("header:X-Fhir-Mssql-Principal".into()),
    ...
    // A read that found nothing is still a disclosure attempt worth
    // recording.
