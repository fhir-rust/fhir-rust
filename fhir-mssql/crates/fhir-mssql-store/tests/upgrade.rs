...
///
/// On this engine the whole upgrade is one transaction (`M14.39`), so there is
/// no partially-carried state for a test to observe in the first place.
#[tokio::test]
async fn reshred_carries_data_across_a_moved_column() {
    let Some(full) = sampled("fhir_mssql_up_reshred", &["Patient"]) else {
        eprintln!("skipping: no map");
        return;
    };
    let Some(d) = dsn() else {
    ...
    let doc = json!({"resourceType": "Patient", "id": "mb",
                     ...
                     "name": [{"family": "Twin"}]});
    store
        .put(&doc, &fhir_mssql_store::Audit::default())
        .await
        .expect("seed");
    let before = store
        .history("Patient", "mb")
...
async fn a_second_reshred_upgrade_carries_nothing() {
    let Some(full) = sampled("fhir_mssql_up_reshred2", &["Patient"]) else {
        eprintln!("skipping: no map");
        return;
    };
    let Some(d) = dsn() else {
    ...
    let opts = fhir_mssql_store::UpgradeOpts {
        allow_destructive: true,
        reshred_moved: true,
    };
    let store = MsSqlStore::connect(&d, Arc::clone(&moved))
        .await
        .expect("connect moved");
    let first = store
    ...
    let store = MsSqlStore::connect(&d, moved).await.expect("reconnect");
    let second = store
        .upgrade_with("moved-sum", opts)
        .await
