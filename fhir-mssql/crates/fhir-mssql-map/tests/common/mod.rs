//! Finding the live SQL Server server, in one place.
//!
...
//! test read `FHIR_MSSQL_TEST_DSN` itself, printed `skipping` to stderr and returned;
...
//!
//! **`FHIR_MSSQL_REQUIRE_DB` reached one test file, not the suite.** The
...
//!
//! 1. `FHIR_MSSQL_TEST_DSN` when set — CI, and anyone who ran
...
//!    unless `FHIR_MSSQL_REQUIRE_DB` is set, in which case a skip is a
...
fn resolve() -> Option<String> {
    if let Ok(d) = std::env::var("FHIR_MSSQL_TEST_DSN")
        && !d.trim().is_empty()
    {
        return Some(d);
    }
    if !listening() {
        assert!(
            !require_db(),
            "FHIR_MSSQL_REQUIRE_DB is set, but no server is reachable at \
             {HOST}:{PORT} and FHIR_MSSQL_TEST_DSN is unset. Refusing to skip: \
             this run was supposed to be live."
        );
        eprintln!(
            "\n\
             ==> fhir-mssql live tests SKIPPED: nothing is listening on {HOST}:{PORT}\n\
             ==>   start one:  cd fhir-mssql && ./scripts/db.sh up\n\
             ==>   or point at your own:  export FHIR_MSSQL_TEST_DSN=<dsn>\n"
        );
        return None;
    }
    eprintln!("==> fhir-mssql live tests using the db.sh container at {HOST}:{PORT}");
    Some(format!(
        "server=tcp:{HOST},{PORT};user=sa;password=Fhir-Mssql-Local-2026!;\
