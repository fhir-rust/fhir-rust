...
`fhir-mssql init --upgrade` for the new digest, tag, and counter-signature
...
`FHIR_MSSQL_CHAIN_KEY` and never written to the database or logged. It sits
...
additive: retired keys stay loadable via `FHIR_MSSQL_CHAIN_KEYS_RETIRED` and
...
**Added — `fhir-mssql chain-witness` and automatic checkpoints.** A tag proves a
...
fhir-mssql cannot enforce that and does not claim to.
...
version of fhir-mssql that had already shipped.
...
accents; fhir-mssql shipped `ILIKE`, which ignores only case, so `family=muller`
...
too low" was "rebuild it". `--pool-size` beats `FHIR_MSSQL_POOL_SIZE`, so a
...
`fhir_mssql_request_latency_micros_total`, which with a request count gives only
...
`fhir_mssql_request_latency_seconds` is now a Prometheus histogram over the
...
- Pool size is configurable via `FHIR_MSSQL_POOL_SIZE` instead of compiled in.
- `crates/fhir-mssql-store/tests/concurrency.rs`: torn reads, racing conditional
  creates, and racing `If-Match` updates (T11.6).

**Added — the audit trail (M3.15–M3.17, §12)**

fhir-mssql still does not authenticate; the perimeter does. But "authentication
...
- Every **read** appends a disclosure record to `fhir_mssql_access_log`: read,
...
- History is **tamper-evident**: a per-resource SHA-256 chain
  ...
  `now()` and cannot race the read of the previous hash. `fhir-mssql
...
The restriction was never fhir-mssql's: the published `fhir` 1.2.0 could not
...
fhir-mssql's floor moves to `fhir = "1.2.1"` with `features = ["r3", "r4"]`, and
...
- The CapabilityStatement now declares `versioning`, `readHistory`,
  ...
  `security` block stating plainly that fhir-mssql verifies no identity (A7.12).
- CI gains a supply-chain job (cargo-deny, CycloneDX SBOM) and `deny.toml`
  (O10.10).
- `book/src/trust-boundary.md`: what fhir-mssql guarantees, what the deployment
...
- FHIR REST server (`fhir-mssql serve`): every installed version mounted,
