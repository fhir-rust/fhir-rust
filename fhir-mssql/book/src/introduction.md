...
>
> What several chapters still call `fhir-mssql serve` is really
> **`fhir-loco`** — a separate crate (Loco.rs, Axum, Tokio, Hyper) that mounts
> a FHIR REST API over a store. `fhir-mssql` itself is a **library**: no binary,
...
fhir-mssql stores [FHIR](https://hl7.org/fhir/) resources in SQL Server 2016 or later as
