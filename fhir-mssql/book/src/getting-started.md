...
```sh
cargo install --path crates/fhir-mssql
export PGHOST=localhost PGUSER=you PGDATABASE=clinic

fhir-mssql init --fhir-version r5      # install the generated schema (~6 s)
fhir-mssql load export/*.ndjson        # load NDJSON / Bundles / single files
fhir-mssql serve                       # FHIR REST API on 127.0.0.1:8080
```
...
```sh
fhir-mssql transform patient.json      # show exactly which rows a resource becomes
fhir-mssql search Patient family=Smith birthdate=ge1970
fhir-mssql get Patient example         # reconstruct one resource
fhir-mssql export Patient > patients.ndjson
```
