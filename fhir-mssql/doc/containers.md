...
cargo test -p fhir-mssql-store --test live -- --nocapture --test-threads=1
```
...
Set `FHIR_MSSQL_SPEC_DIR` to point somewhere else. The tests resolve the same two
...
| Variable | Meaning |
|---|---|
| `FHIR_MSSQL_TEST_DSN` | Set by `dsn`. Without it the live tests skip. |
| `FHIR_MSSQL_SPEC_DIR` | FHIR definitions directory. |
| `FHIR_MSSQL_CORPUS_DIR` | Example corpus directory. |
| `FHIR_MSSQL_TEST_CORPUS_LIMIT` | Examples per version (default 400; use 10–25 for a fast loop). |
| `FHIR_MSSQL_BENCH` | Set to a row count to enable the benchmark test. |

There is no `FHIR_MSSQL_CHAIN_KEY` — an earlier revision of this table
...
coverage this crate has (`mssql_store.rs` builds a `KeyRing` directly with a
fixed test key via `ChainKey::from_hex`/`with_chain_keys`) runs
