...
Ground-up rewrite of fhir-mariadb: fully normalized relational storage of FHIR®
R3/R4/R5 in MariaDB 11.4. The REST server is a separate crate — `fhir-loco` — and
...
- **D2 — All resource types, three versions.** R5 5.0.0 (default), R4 4.0.1,
  R3 3.0.2, each complete, each in its own MariaDB database (`r5`/`r4`/`r3`).
...
- **D13 — Auth is perimeter, not core.** This library implements no
  ...
  **Amended by D15:** the perimeter authenticates, but fhir-mariadb must still
  *record* who acted.
- **D14 — Workspace layout.** One cargo workspace:
  `fhir-mariadb-map` (relational map types + generic shred/reconstruct engine),
  `fhir-mariadb-gen` (spec → DDL + map), `fhir-mariadb-store` (MariaDB 11.4 layer:
  init/load/search/history). **There is no `fhir-mariadb-server` and no
  `fhir-mariadb` CLI binary** — this decision was the ancestor project's, and the
...
- **D19 — Normalize for search, keep the original for truth.** Accent- and
  ...
  matched against.
- **D20 — Encrypt the database link by default.** `FHIR_MARIADB_SSL_MODE`,
  in MariaDB's own `--ssl-mode` vocabulary — deliberately not libpq's
  `sslmode` or `PGSSLROOTCERT`, which this port does not read — defaulting
  to `VERIFY_IDENTITY` and applied by `connect_with` (O10.7, **F-54**,
...
- **R1 — Schema scale.** ~3,000+ tables per version; `init` time, catalog
  ...
  Mitigation: per-version MariaDB database, benchmarks from milestone 1.
  Note the DDL is **not** applied in one transaction — MariaDB commits DDL
...
- **R9 — Snapshot reads under long transactions.** REPEATABLE READ readers
  ...
  today (`max_statement_time` would be this engine's equivalent), so
...
- **M1 — Engine proven (R5, vertical slice).** Generator produces DDL + map
  ...
  example; `init`/`load`/`transform`/`export` work; live-MariaDB 11.4
...
- **M2 — History + CRUD semantics.** version_id/history/soft delete;
  transactional writes; ETag concurrency; `fhir_mariadb_meta` and idempotent init.
