...
```sh
cargo install --path crates/fhir-mariadb
export PGHOST=localhost PGUSER=you PGDATABASE=clinic

fhir-mariadb init --fhir-version r5      # install the generated schema (~6 s)
fhir-mariadb load export/*.ndjson        # load NDJSON / Bundles / single files
fhir-mariadb serve                       # FHIR REST API on 127.0.0.1:8080
```
...
```sh
fhir-mariadb transform patient.json      # show exactly which rows a resource becomes
fhir-mariadb search Patient family=Smith birthdate=ge1970
fhir-mariadb get Patient example         # reconstruct one resource
fhir-mariadb export Patient > patients.ndjson
```
