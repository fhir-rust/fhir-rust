//! fhir-mariadb-gen: generates the fhir-mariadb relational map from the official FHIR
...
pub fn generate(definitions_dir: &Path, schema: &str) -> Result<RelMap, GenError> {
    ...
    // G2.6a: after every column is placed, so the bound is a fact of the
    // finished map (F-90).
    build::assert_row_budget(&map)?;
