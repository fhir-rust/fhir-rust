//! Finding the live MariaDB server, in one place.
//!
...
//! test read `FHIR_MARIADB_TEST_DSN` itself, printed `skipping` to stderr and returned;
...
//!
//! **`FHIR_MARIADB_REQUIRE_DB` reached one test file, not the suite.** The
...
//!
//! 1. `FHIR_MARIADB_TEST_DSN` when set — CI, and anyone who ran
...
//!    unless `FHIR_MARIADB_REQUIRE_DB` is set, in which case a skip is a
...
pub fn require_db() -> bool {
    std::env::var("FHIR_MARIADB_REQUIRE_DB").is_ok_and(|v| v != "0" && !v.is_empty())
}

fn resolve() -> Option<String> {
    if let Ok(d) = std::env::var("FHIR_MARIADB_TEST_DSN")
        && !d.trim().is_empty()
    {
        return Some(d);
    }
    if !listening() {
        assert!(
            !require_db(),
            "FHIR_MARIADB_REQUIRE_DB is set, but no server is reachable at \
             {HOST}:{PORT} and FHIR_MARIADB_TEST_DSN is unset. Refusing to skip: \
             this run was supposed to be live."
        );
        eprintln!(
            "\n\
             ==> fhir-mariadb live tests SKIPPED: nothing is listening on {HOST}:{PORT}\n\
             ==>   start one:  cd fhir-mariadb && ./scripts/db.sh up\n\
             ==>   or point at your own:  export FHIR_MARIADB_TEST_DSN=<dsn>\n"
        );
        return None;
    }
    set_if_unset("FHIR_MARIADB_SSL_MODE", "DISABLED");
    eprintln!("==> fhir-mariadb live tests using the db.sh container at {HOST}:{PORT}");
    Some(format!("mysql://root@{HOST}:{PORT}"))
}
