...
//!   between them reconstructs a resource that never existed. MariaDB's default
...
async fn fresh(schema: &str) -> Option<Arc<MariaDbStore>> {
...
async fn reads_never_tear_under_concurrent_writes() {
    let Some(store) = fresh("fhir_mariadb_conc_torn").await else {
        eprintln!("skipping: set FHIR_MARIADB_TEST_DSN to run");
        return;
    };
    store
        .put(&patient("torn", 0), &fhir_mariadb_store::Audit::default())
        .await
        .expect("seed");
    ...
    let writer = tokio::spawn({
        ...
        async move {
            for n in 1..=120 {
                store
                    .put(&patient("torn", n), &fhir_mariadb_store::Audit::default())
                    .await
                    .expect("write");
            }
        }
...
async fn racing_writers_get_distinct_versions_and_a_verifiable_chain() {
    let Some(store) = fresh("fhir_mariadb_conc_ver").await else {
        eprintln!("skipping: set FHIR_MARIADB_TEST_DSN to run");
        return;
    };
    store
        .put(&patient("ver", 0), &fhir_mariadb_store::Audit::default())
        .await
        .expect("seed");
    ...
    let racers: Vec<_> = (1..=WRITERS)
        .map(|n| {
            ...
            tokio::spawn(async move {
                store
                    .put(&patient("ver", n), &fhir_mariadb_store::Audit::default())
                    .await
            })
        })
        .collect();
