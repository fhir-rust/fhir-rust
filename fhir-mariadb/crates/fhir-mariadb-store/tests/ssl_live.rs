...
//! certificate does not check out — which is exactly what a stock MariaDB
...
//!
//! Needs `FHIR_MARIADB_TEST_DSN`. Skips loudly without one, and fails rather than
//! skips when `FHIR_MARIADB_REQUIRE_DB` is set (`T11.12`, `T11.13`).
...
macro_rules! skip_or_fail {
    ($reason:expr) => {{
        assert!(
            !require_db(),
            "FHIR_MARIADB_REQUIRE_DB is set, so this test must run: {}",
            $reason
        );
...
fn map() -> Arc<RelMap> {
    Arc::new(RelMap {
        ...
        schema: "fhir_mariadb_ssltest".into(),
...
async fn tls_is_configurable_and_verification_is_not_a_no_op() {
    let Some(dsn) = dsn() else {
        skip_or_fail!("set FHIR_MARIADB_TEST_DSN to run");
    };
    ...
    unsafe { std::env::remove_var("FHIR_MARIADB_SSL_CA") };

    // 1. The server is reachable at all. Without this, test 3's failure could
    //    just be "no server".
    MariaDbStore::connect_with(&dsn, map(), SslMode::Disabled)
        .await
        .expect(
    ...
    MariaDbStore::connect_with(&dsn, map(), SslMode::Required)
        .await
    ...
    let verified = MariaDbStore::connect_with(&dsn, map(), SslMode::VerifyIdentity).await;
    assert!(
        verified.is_err(),
        "VERIFY_IDENTITY accepted a stock MariaDB container's self-signed \
