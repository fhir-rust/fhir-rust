...
//!
//! Two things here are MariaDB's doing and not choices, and each has its own test:
//!
//! - **No transactional DDL.** A failed upgrade leaves a partial schema. That
//!   cannot be prevented, so it is reported (`M14.35`).
//! - **`CREATE INDEX` is not idempotent as emitted.** MariaDB *does* have
//!   `CREATE INDEX IF NOT EXISTS`, unlike MySQL — but `ddl.rs` does not use it,
//!   so reconciling the access-log indexes wholesale fails on the second run
//!   with `Duplicate key name` all the same (audit **F-28**).
//!   `a_second_upgrade_is_a_no_op` is what catches that.
//!
//! Needs `FHIR_MARIADB_TEST_DSN`; `scripts/db.sh up` prints it.

use std::sync::Arc;

use fhir_mariadb_map::model::{RelMap, TargetKind};
...
async fn seeded(schema: &str) -> Option<(RelMap, MariaDbStore)> {
    ...
    let store = MariaDbStore::connect(&dsn()?, Arc::new(old))
        .await
        .expect("connect");
    ...
    let new_store = MariaDbStore::connect(&dsn()?, Arc::new(full.clone()))
        .await
        .expect("connect new");
    Some((full, new_store))
}
...
async fn upgrade_applies_the_additive_diff_and_keeps_existing_data() {
    let Some((_, store)) = seeded("fhir_mariadb_up_add").await else {
...
async fn rows_written_before_the_folded_column_are_backfilled() {
    let Some((_, store)) = seeded("fhir_mariadb_up_fold").await else {
        eprintln!("skipping: set FHIR_MARIADB_TEST_DSN to run");
        return;
    };
    let report = store.upgrade("new-sum", false).await.expect("upgrade");
...
///
/// This is the one that catches **F-28**: the emitted access-log indexes carry
/// no `IF NOT EXISTS`, so reconciling `schema_wide_objects` wholesale fails here
/// with `Duplicate key name` — even though MariaDB would support the guard.
#[tokio::test]
async fn a_second_upgrade_is_a_no_op() {
    let Some((_, store)) = seeded("fhir_mariadb_up_noop").await else {
        eprintln!("skipping: set FHIR_MARIADB_TEST_DSN to run");
        return;
    };
...
async fn the_backfill_is_resumable() {
    let Some((_, store)) = seeded("fhir_mariadb_up_resume").await else {
        eprintln!("skipping: set FHIR_MARIADB_TEST_DSN to run");
        return;
    };
    let first = store
...
async fn destructive_changes_are_refused_without_the_flag() {
    let Some(full) = sampled("fhir_mariadb_up_drop", &["Patient"]) else {
    ...
    let store = MariaDbStore::connect(&d, Arc::new(pre_folding(&full)))
        .await
...
async fn a_column_type_change_refuses() {
    let Some(full) = sampled("fhir_mariadb_up_type", &["Patient"]) else {
        eprintln!("skipping: no r5 relmap asset");
        return;
    };
    let Some(d) = dsn() else {
    ...
    let col = {
        ...
        c.ty = fhir_mariadb_map::model::ColTy::Text;
        c.name.clone()
    };
    let store = MariaDbStore::connect(&d, Arc::new(changed))
        .await
        .expect("connect");
    let err = store
...
async fn an_install_without_a_stored_map_asset_says_so() {
    let Some((full, store)) = seeded("fhir_mariadb_up_noasset").await else {
        eprintln!("skipping: set FHIR_MARIADB_TEST_DSN to run");
        return;
    };
    ...
    // separate connection because the store deliberately exposes no raw exec.
    {
        ...
        c.query_drop(
            "DELETE FROM `fhir_mariadb_up_noasset`.`fhir_mariadb_meta` WHERE `key` = 'map_asset'",
        )
        .await
        .expect("delete asset");
    }
...
async fn upgrading_an_uninstalled_schema_says_it_is_not_installed() {
    let Some(full) = sampled("fhir_mariadb_up_absent", &["Patient"]) else {
        eprintln!("skipping: no r5 relmap asset");
        return;
    };
    let Some(d) = dsn() else {
    ...
    let store = MariaDbStore::connect(&d, Arc::new(full))
        .await
        .expect("connect");
    store.drop_schema().await.expect("drop");
    let err = store
...
async fn reshred_carries_data_across_a_moved_column() {
    let Some(full) = sampled("fhir_mariadb_up_reshred", &["Patient"]) else {
        eprintln!("skipping: no dsn or map");
        return;
    };
    let Some(d) = dsn() else {
    ...
    let doc = json!({"resourceType": "Patient", "id": "mb",
                     ...
                     "name": [{"family": "Twin"}]});
    store
        .put(&doc, &fhir_mariadb_store::Audit::default())
        .await
        .expect("seed");
    let before = store
        .history("Patient", "mb")
    ...
    let report = store
        .upgrade_with(
            "moved-sum",
            fhir_mariadb_store::UpgradeOpts {
...
async fn a_second_reshred_upgrade_carries_nothing() {
    let Some(full) = sampled("fhir_mariadb_up_reshred2", &["Patient"]) else {
        eprintln!("skipping: no dsn or map");
        return;
    };
    let Some(d) = dsn() else {
    ...
    let opts = fhir_mariadb_store::UpgradeOpts {
        allow_destructive: true,
        reshred_moved: true,
    };
    let store = MariaDbStore::connect(&d, Arc::clone(&moved))
        .await
        .expect("connect moved");
    let first = store
    ...
    let store = MariaDbStore::connect(&d, moved).await.expect("reconnect");
    let second = store
        .upgrade_with("moved-sum", opts)
        .await
