//! Search-query construction for MySQL (spec section 14, T64).
//!
//! Forked from the inherited PostgreSQL builder rather than shared with it: the
//! ports are independent by design (M14.0a), and parameterising one builder at
//! every point the dialects differ would be more coupling than either engine
//! gains.
//!
//! What differs, and why:
//!
//! - **Placeholders are plain `?`, not `$n`.** MySQL binds strictly in order,
//!   so every value must be emitted exactly once, in the order it is pushed —
//!   which is what `b()` already guarantees. The SQLite fork can use numbered
//!   `?n`; here the numbering would be a lie.
//! - **Identifiers are backquoted.** `"` is a string delimiter unless the server
//!   runs in `ANSI_QUOTES` mode, which a query must not assume.
//! - **`NULLS LAST` does not exist.** It is emulated with a leading
//!   `ISNULL(col)` sort term, which sorts nulls after values.
//! - **Numbers compare through `CAST(… AS DECIMAL(65,30))`.** The column holds
//!   the exact lexical form, because M3.6 requires that to survive round-trip,
//!   and text compares lexicographically — under which "9" > "10". `DECIMAL`
//!   rather than `DOUBLE` because MySQL has it and it is exact within range;
//!   the SQLite fork uses `REAL` because SQLite does not. The two ports need not
//!   agree, and here the better tool is available.
//! - **Dates need no cast.** They are `DATETIME(6)`, and the bound value is the
//!   same fixed-width UTC text the store writes, which MySQL coerces correctly.
//! - **`ILIKE` becomes `LIKE`.** Correctness does not rest on the engine's
//!   collation: every non-exact string comparison runs against the folded column
//!   whose value came from Rust's `fold::fold` (M14.5).

use fhir_mariadb_map::fold::{fold, prefix_upper};
use fhir_mariadb_map::model::{RelMap, ResourceMap, SearchDef, TargetKind};
...
pub fn build_search_sql(
    ...
    for key in sort {
        ...
        // MySQL has no NULLS LAST; ISNULL() sorts 0 before 1, so nulls land
        // after values under either direction.
        order.push(format!(
            "ISNULL({col}), {col} {}",
...
fn param_predicate(
    ...
    if let Some((head, rest)) = rawname.split_once('.') {
        ...
        for t in &def.targets {
            ...
            let core = format!(
                "({ref_alias}.`{c_type}` = ? AND EXISTS (SELECT 1 FROM                  `{schema}`.`{tbase}` {t_alias} WHERE {t_alias}.`id` =                  {ref_alias}.`{c_id}` AND {inner}))"
            );
            match wrap {
...
fn target_pred(
    def: &SearchDef,
    t: &fhir_mariadb_map::model::SearchTarget,
    value: &str,
    modifier: Option<&str>,
    binds: &mut Vec<String>,
) -> Result<String, StoreError> {
    ...
    match &t.kind {
        TargetKind::Str { col, norm } => {
            ...
            // back to bare `LIKE`, which under MySQL's default collation is
            // case-insensitive and accent-insensitive but not the same fold
            // Rust applies — so "Muller" may or may not find "Müller"
            // depending on the column's collation. Regenerating the map is the
            // fix, not papering over it here.
        ...
        TargetKind::Number { col } => {
            ...
            Ok(format!(
                "CAST(«c».`{col}` AS DECIMAL(65,30)) {op} CAST({} AS DECIMAL(65,30))",
                b(binds, num)
            ))
        }
        TargetKind::Quantity {
            ...
            let mut pred = format!(
                "CAST(«c».`{vcol}` AS DECIMAL(65,30)) {op} CAST({} AS DECIMAL(65,30))",
                b(binds, num)
            );
...
fn date_pred(
    ...
    let hi_of = |binds: &mut Vec<String>| match &hi {
        ...
        None => {
            ...
            format!("DATE_ADD({lo_b}, INTERVAL 1 SECOND)")
        }
        Some(h) => bind(binds, h),
    };
    ...
    Ok(match prefix {
        "eq" | "ne" => {
            let inner = match &end {
                ...
                Some(e) => {
                    ...
                    let lo_b = bind(binds, &lo_val);
                    format!(
                        "({lo_col} < {hi_b} AND coalesce({e}, '9999-12-31 23:59:59.999999') >= {lo_b})"
                    )
                }
                None => {
                    let lo_b = bind(binds, &lo_val);
        ...
        "gt" | "sa" => {
            let hi_b = hi_of(binds);
            match &end {
                Some(e) => format!("coalesce({e}, '9999-12-31 23:59:59.999999') >= {hi_b}"),
        ...
        "ge" => {
            let lo_b = bind(binds, &lo_val);
            match &end {
                Some(e) => format!("coalesce({e}, '9999-12-31 23:59:59.999999') >= {lo_b}"),
