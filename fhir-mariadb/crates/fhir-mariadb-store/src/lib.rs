//! fhir-mariadb-store: the PostgreSQL layer. Applies generated DDL, writes shredded
...
pub enum StoreError {
    ...
    Db(String),
    #[error("shred: {0}")]
    Shred(#[from] fhir_mariadb_map::ShredError),
...
///
/// fhir_mariadb does not authenticate — that is the perimeter's job (plan D13) —
