//! DDL emission: the map, rendered as MariaDB CREATE statements.
...
const ID_COL: &str = "VARCHAR(64) COLLATE utf8mb4_nopad_bin";

/// MariaDB type mapping for the map's column types (M14.14).
/// Does this dialect need the unbounded-string adjuncts (`U1`, `U9`)?
///
/// a prefix index over `TEXT` covers it, so U9 forbids the adjuncts.
///
...
pub fn col_sql(ty: ColTy) -> &'static str {
    match ty {
        ...
        ColTy::Numeric => "TEXT",
        ColTy::Text => "TEXT",
        ColTy::TextC => "TEXT COLLATE utf8mb4_nopad_bin",
        ...
        ColTy::TextIdx => "VARCHAR(450) COLLATE utf8mb4_nopad_bin",
...
/// Is this type stored as a text kind that MariaDB cannot index whole?
...
pub fn ddl_in(map: &RelMap, schema: &str) -> Vec<String> {
    ...
    out.push(format!(
        "CREATE TABLE `{s}`.`fhir_mariadb_meta` (`key` VARCHAR(191) NOT NULL PRIMARY KEY, `value` LONGTEXT NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
    ));
...
/// because MariaDB has no `CREATE INDEX IF NOT EXISTS`. Re-applying this list
...
///
/// MariaDB supports `ADD COLUMN IF NOT EXISTS`, so these are idempotent and the
/// caller may apply them blindly — the same contract the PostgreSQL original
/// offered. The sibling fhir-mysql port cannot do this and has to diff against
...
///
/// `actor` carries no `DEFAULT` because MariaDB forbids defaults on `TEXT`
...
fn access_log_table(s: &str) -> String {
    format!(
        "CREATE TABLE IF NOT EXISTS `{s}`.`fhir_mariadb_access_log` (\n\
...
fn countersign_table(s: &str) -> String {
    format!(
        "CREATE TABLE IF NOT EXISTS `{s}`.`fhir_mariadb_countersign` (\n\
         \x20 `seq` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,\n\
         \x20 `rtype` VARCHAR(64) NOT NULL,\n\
         \x20 `id` {ID_COL} NOT NULL,\n\
         \x20 `version_id` BIGINT NOT NULL,\n\
         \x20 `row_mac` VARCHAR(191) COLLATE utf8mb4_nopad_bin NOT NULL,\n\
         \x20 `signed_at` DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),\n\
         \x20 `actor` TEXT NOT NULL,\n\
         \x20 `reason` TEXT NOT NULL,\n\
         \x20 UNIQUE KEY `fhir_mariadb_countersign_uq` (`rtype`, `id`, `version_id`, `row_mac`)\n\
         ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
    )
}
...
fn access_log_indexes(s: &str) -> Vec<String> {
    ...
    let t = "fhir_mariadb_access_log";
    vec![
        format!(
            "CREATE INDEX `fhir_mariadb_access_log_subject_ix` ON `{s}`.`{t}` (`rtype`, `id`, `ts`)"
        ),
        format!(
            "CREATE INDEX `fhir_mariadb_access_log_actor_ix` ON `{s}`.`{t}` (`actor`(191), `ts`)"
        ),
        format!("CREATE INDEX `fhir_mariadb_access_log_ts_ix` ON `{s}`.`{t}` (`ts`)"),
    ]
}
...
///
/// Two triggers rather than PostgreSQL's one, because MariaDB allows a trigger
/// to name only a single event. But `CREATE OR REPLACE TRIGGER` is MariaDB's
/// own, so each trigger is a single idempotent statement — exactly the
/// PostgreSQL original's shape, and closer to it than the sibling fhir-mysql
/// port can get: MySQL has neither `OR REPLACE` nor `IF NOT EXISTS` for
/// triggers and must emit a drop first.
///
...
/// `@fhir_mariadb_erasure` is set, which is how `fhir-mariadb purge` performs a
...
pub fn append_only_triggers(s: &str, table: &str) -> Vec<String> {
    ...
    vec![
        format!(
            "CREATE OR REPLACE TRIGGER `{s}`.`{upd}` BEFORE UPDATE ON `{s}`.`{table}` FOR EACH ROW \
             SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = \
             'fhir-mariadb: UPDATE forbidden; history is append-only (M3.17)'"
        ),
        format!(
            "CREATE OR REPLACE TRIGGER `{s}`.`{del}` BEFORE DELETE ON `{s}`.`{table}` FOR EACH ROW \
             BEGIN \
             IF COALESCE(@fhir_mariadb_erasure, '') <> 'on' THEN \
             SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = \
             'fhir-mariadb: DELETE forbidden; history is append-only (M3.17)'; \
             END IF; \
             END"
        ),
    ]
...
///
/// MariaDB cannot index a `TEXT` column without a prefix length, and the whole
...
/// Render an index's column list, adding a prefix length to each column MariaDB
/// cannot index whole.
...
pub fn create_table(schema: &str, rm: &ResourceMap, t: &Table) -> String {
    ...
    match t.kind {
        ...
        TableKind::Ext => {
            ...
            // includes two unbounded text columns and cannot be a MariaDB primary
...
/// MariaDB, not just the table, so they get the same 63-byte fitting as indexes.
...
mod tests {
    ...
    fn exact_text_keeps_a_binary_collation() {
        // MariaDB's default collation is accent- and case-insensitive, which would
        ...
        assert!(col_sql(ColTy::TextC).contains("utf8mb4_nopad_bin"));
    }
    ...
    fn append_only_triggers_are_single_idempotent_statements() {
        ...
        // `CREATE OR REPLACE` is MariaDB's, so no companion DROP is needed and
        // each trigger is one statement — the PostgreSQL original's shape.
        assert_eq!(t.len(), 2);
        assert!(t.iter().all(|s| s.starts_with("CREATE OR REPLACE TRIGGER")));
        assert!(t.iter().all(|s| !s.contains("DROP TRIGGER")));
        assert!(t[0].contains("BEFORE UPDATE ON"));
        assert!(t[1].contains("BEFORE DELETE ON"));
        // UPDATE has no escape hatch; DELETE has exactly one.
        assert!(!t[0].contains("fhir_mariadb_erasure"));
        assert!(t[1].contains("@fhir_mariadb_erasure"));
        for stmt in [&t[0], &t[1]] {
            assert!(stmt.contains("SIGNAL SQLSTATE '45000'"));
            // MESSAGE_TEXT is truncated at 128 characters by MariaDB.
