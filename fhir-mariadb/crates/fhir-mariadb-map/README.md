...
```rust
use std::sync::Arc;
use fhir_mariadb_map::{RelMap, shred, reconstruct};
...
| `ddl` | **the only dialect-specific file** — emits MariaDB DDL |
...
| Column | Purpose |
| --- | --- |
| `<col>_norm` | the folded form, so prefix search is case- and accent-insensitive (`TEXT COLLATE utf8mb4_nopad_bin`) |
