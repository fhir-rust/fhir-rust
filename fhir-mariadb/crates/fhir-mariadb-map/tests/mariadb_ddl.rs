...
//!
//! Needs `FHIR_MARIADB_TEST_DSN`, e.g.
...
//! dropped per run.
//!
//! The emitted DDL uses MariaDB-specific syntax where MariaDB does it better —
//! `CREATE OR REPLACE TRIGGER`, `ADD COLUMN IF NOT EXISTS`,
//! `utf8mb4_nopad_bin` — and is deliberately *not* portable to MySQL (M14.0a,
//! M14.0b). That is why this test runs against a real server rather than
//! trusting that the sibling port's verification carries over.

use std::process::Command;

/// `(host, port, user)` parsed loosely from a DSN, or `None` to skip.
fn dsn() -> Option<(String, String, String)> {
    let raw = std::env::var("FHIR_MARIADB_TEST_DSN").ok()?;
...
fn relmap(version: &str) -> Option<fhir_mariadb_map::model::RelMap> {
    let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("assets")
        .join(format!("fhir-mariadb-relmap-{version}.json.gz"));
    let bytes = std::fs::read(path).ok()?;
    fhir_mariadb_map::model::RelMap::from_gz_bytes(&bytes).ok()
}
...
fn install(version: &str) {
    ...
    let schema = format!("fhir_mariadb_ddltest_{version}");
    let _ = run(&format!("DROP SCHEMA IF EXISTS `{schema}`"));

    // Creating every InnoDB table with its foreign key takes tens of minutes on
    // a laptop — MariaDB is far slower at this than PostgreSQL or SQLite, which is
    ...
    // every column type, and `FHIR_MARIADB_DDL_FULL=1` installs all of them.
    let full = std::env::var("FHIR_MARIADB_DDL_FULL").is_ok();
    let mut map = map;
    if !full {
        let limit: usize = std::env::var("FHIR_MARIADB_DDL_LIMIT")
            .ok()
    ...
    let statements = fhir_mariadb_map::ddl::ddl_in(&map, &schema);
    assert!(
        statements.len() > 50,
...
fn append_only_triggers_actually_refuse_writes() {
    ...
    let schema = "fhir_mariadb_trgtest";
    let _ = run(&format!("DROP SCHEMA IF EXISTS `{schema}`"));
    let mut setup = vec![
    ...
    setup.extend(fhir_mariadb_map::ddl::append_only_triggers(
        schema,
        "patient_history",
    ));
    setup.push(format!(
...
fn exact_collation_does_not_pad_trailing_spaces() {
    ...
    if dsn().is_none() || mysql_cli().is_none() {
        eprintln!("skipping: set MARIADB_TEST_DSN to run");
        return;
    }
    let ty = fhir_mariadb_map::ddl::col_sql(fhir_mariadb_map::model::ColTy::TextC);
    assert!(
        ty.contains("utf8mb4_nopad_bin"),
        "unexpected collation: {ty}"
    );

    let out = run(
        "SELECT ('Smith' COLLATE utf8mb4_nopad_bin) = ('Smith ' COLLATE utf8mb4_nopad_bin) AS padded",
    )
    .expect("collation comparison runs");
    let last = out.lines().last().unwrap_or("").trim().to_string();
    assert_eq!(
        last, "0",
        "utf8mb4_nopad_bin treated 'Smith' and 'Smith ' as equal — it pads, so it is the wrong collation"
    );
}
