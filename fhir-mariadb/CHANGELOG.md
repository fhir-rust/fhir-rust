...
`FHIR_MARIADB_SSL_MODE` takes MariaDB's own `--ssl-mode` values —
`DISABLED`, `REQUIRED`, `VERIFY_CA`, `VERIFY_IDENTITY` — and defaults to
`VERIFY_IDENTITY`. `FHIR_MARIADB_SSL_CA` names a root certificate for a
private CA. `Store::connect_with` takes a mode explicitly.

Note that MariaDB's own `REQUIRED` encrypts without validating anything, so it
...
prints `FHIR_MARIADB_SSL_MODE=DISABLED` next to the DSN for that case.
...
`fhir-mariadb init --upgrade` for the new digest, tag, and counter-signature
columns; existing rows keep verifying, with the new chains reported as
beginning where their first digest appears.


**Changed — hash chains are computed by fhir-mariadb, not by PostgreSQL.** The
...
`FHIR_MARIADB_CHAIN_KEY` and never written to the database or logged. It sits
...
additive: retired keys stay loadable via `FHIR_MARIADB_CHAIN_KEYS_RETIRED` and
...
**Added — `fhir-mariadb chain-witness` and automatic checkpoints.** A tag proves a
...
fhir-mariadb cannot enforce that and does not claim to.
...
version of fhir-mariadb that had already shipped.
...
without `--allow-insecure-db`. fhir-mariadb's `require` validates the server
...
accents; fhir-mariadb shipped `ILIKE`, which ignores only case, so `family=muller`
...
too low" was "rebuild it". `--pool-size` beats `FHIR_MARIADB_POOL_SIZE`, so a
...
`fhir_mariadb_request_latency_micros_total`, which with a request count gives only
...
`fhir_mariadb_request_latency_seconds` is now a Prometheus histogram over the
...
- Pool size is configurable via `FHIR_MARIADB_POOL_SIZE` instead of compiled in.
- `crates/fhir-mariadb-store/tests/concurrency.rs`: torn reads, racing conditional
  creates, and racing `If-Match` updates (T11.6).

**Added — the audit trail (M3.15–M3.17, §12)**

fhir-mariadb still does not authenticate; the perimeter does. But "authentication
...
- Every **read** appends a disclosure record to `fhir_mariadb_access_log`: read,
...
- History is **tamper-evident**: a per-resource SHA-256 chain
  ...
  `now()` and cannot race the read of the previous hash. `fhir-mariadb
...
The restriction was never fhir-mariadb's: the published `fhir` 1.2.0 could not
...
fhir-mariadb's floor moves to `fhir = "1.2.1"` with `features = ["r3", "r4"]`, and
...
- The CapabilityStatement now declares `versioning`, `readHistory`,
  ...
  `security` block stating plainly that fhir-mariadb verifies no identity (A7.12).
- CI gains a supply-chain job (cargo-deny, CycloneDX SBOM) and `deny.toml`
  (O10.10).
- `book/src/trust-boundary.md`: what fhir-mariadb guarantees, what the deployment
...
- FHIR REST server (`fhir-mariadb serve`): every installed version mounted,
