...
fhir-mariadb is mirrored on GitHub and Codeberg, and both forges run the same
...
| Live MariaDB suite | `ci.yml` (`database`) | `.woodpecker/database.yaml` |
...
extra: most of fhir-mariadb's guarantees are database guarantees — snapshot
...
unencrypted link. MariaDB is a network server, so that gate **does** apply here
and should come back — but it cannot be written honestly until the store speaks
MariaDB (T64), because `SslPolicy` and the plaintext-refusal guard are
store-layer behaviour. Rather than leave a PostgreSQL job pretending to cover
this port, it is removed and tracked as an open task.
## MSRV
