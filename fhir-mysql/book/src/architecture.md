...
- **fhir-mysql-store** — `mysql_async` with a connection pool. Transactional writes with
...
- **fhir-mysql-server** — axum. The FHIR RESTful API, bundle processing,
  generated CapabilityStatements, request ids, metrics.
- **fhir-mysql** — the CLI binary tying it together.
