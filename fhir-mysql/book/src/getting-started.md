...
```sh
cargo install --path crates/fhir-mysql
export PGHOST=localhost PGUSER=you PGDATABASE=clinic

fhir-mysql init --fhir-version r5      # install the generated schema (~6 s)
fhir-mysql load export/*.ndjson        # load NDJSON / Bundles / single files
fhir-mysql serve                       # FHIR REST API on 127.0.0.1:8080
```
...
```sh
fhir-mysql transform patient.json      # show exactly which rows a resource becomes
fhir-mysql search Patient family=Smith birthdate=ge1970
fhir-mysql get Patient example         # reconstruct one resource
fhir-mysql export Patient > patients.ndjson
```
