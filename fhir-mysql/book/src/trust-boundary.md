...
fhir-mysql is a component, not a system. It cannot make a deployment compliant,
and it must not be the reason a deployment cannot be. This chapter states, in
one place, what fhir-mysql guarantees and what the deployment around it has to
provide — because a boundary nobody can point at is not a boundary (spec
PR12.8).

## What fhir-mysql guarantees
...
| Every read is recorded | disclosure row in `fhir_mysql_access_log` | PR12.5 |
...
fhir-mysql does **not** do these, and a deployment that skips them is not safe to
put patient data in:

| Obligation | Why it is not here |
| --- | --- |
| **Authentication** | Identity belongs to the perimeter (plan D13). fhir-mysql accepts a principal asserted by a trusted proxy and records it; it verifies nothing. |
...
fhir-mysql serve \
  ...
  --principal-header X-Fhir-Mysql-Principal \
  --reason-header X-Fhir-Mysql-Purpose \
  --require-principal
```

Each flag is load-bearing:

- Without `--base-url`, paging links are built from the address fhir-mysql bound,
...
```sh
fhir-mysql verify-audit --fhir-version r5
```
...
can also write matching digests. Set `FHIR_MYSQL_CHAIN_KEY` and that stops being
true.

```sh
# 32 bytes minimum. A placeholder like "changeme" would produce tags an
# attacker could reproduce by guessing.
export FHIR_MYSQL_CHAIN_KEY=$(openssl rand -hex 32)
export FHIR_MYSQL_CHAIN_KEY_ID=k1
```
...
```sh
export FHIR_MYSQL_CHAIN_KEY=$(openssl rand -hex 32)   # the new signing key
export FHIR_MYSQL_CHAIN_KEY_ID=k2
export FHIR_MYSQL_CHAIN_KEYS_RETIRED="k1=<previous hex>"   # still verifies
```
...
```sh
fhir-mysql chain-witness --fhir-version r5   # e.g. k1:3f2a…  or  1042:9c81…
```
...
the same compromised account can rewrite, do not. fhir-mysql cannot enforce that
...
must not be. These genuinely conflict, and fhir-mysql resolves it in one direction,
explicitly:

```sh
fhir-mysql purge Patient 1234 --reason "art-17 request #4471" --allow-erasure
```
...
- **The guard is against accident, not against the application.** The
  ...
  `fhir_mysql.erasure`, which is how `purge` works — so application-level SQL
...
```sql
REVOKE UPDATE, DELETE ON ALL TABLES IN SCHEMA r5 FROM fhir_mysql_app;
GRANT SELECT, INSERT ON r5.patient_history TO fhir_mysql_app;  -- and each _history
```
