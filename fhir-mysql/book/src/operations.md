...
fhir-mysql handles PHI. The server binds loopback by default and implements
...
- `/metrics` — Prometheus text: request totals, response classes, and
  `fhir_mysql_request_latency_seconds`, a histogram over the default 1ms–10s
...
(`FHIR_MYSQL_STATEMENT_TIMEOUT_MS`); pool waits are bounded at 2 s, and
exhaustion answers **503 + Retry-After** instead of queueing unboundedly.
`fhir-mysql serve` shuts down gracefully on SIGINT/SIGTERM, draining any queued
...
| `--pool-size` | 16 | Database connections. Overrides `FHIR_MYSQL_POOL_SIZE` |
...
`fhir-mysql init` installs under a staging schema in chunked transactions and
...
`fhir-mysql init --upgrade` diffs the installed map against the current
...
migration. `fhir-mysql drop --yes` removes a version schema in lock-safe
chunks.
