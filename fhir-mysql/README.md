# fhir-mysql — FHIR® in MySQL 8.4, relationally

Store [FHIR](https://hl7.org/fhir/) resources in MySQL 8.4 as **real relational
tables** — typed columns, child tables, foreign keys, and check constraints —
not JSON blobs. Get them back losslessly.

- **Given a FHIR resource**, `fhir-mysql` shreds it into normalized tables
...
> engine, the MySQL 8.4 store, search, history, and the audit chain work, and
> a live MySQL 8.4 gate runs them in CI.
>
...
> asked the driver for a `String` and MySQL returns `Value::Int`/`Value::Date`
...
This is a **library**, not a CLI. There is no `fhir-mysql` binary and no server
crate in this workspace.

```toml
[dependencies]
fhir-mysql-map   = { path = "crates/fhir-mysql-map" }
fhir-mysql-store = { path = "crates/fhir-mysql-store" }
...
use fhir_mysql_store::{Audit, mysql::MySqlStore};
...
## What is specific to MySQL 8.4

Full detail in the [dialect annex](spec/14-mysql-dialect.md); the highlights:
...
- **`TextC` is `TEXT COLLATE utf8mb4_0900_bin`.** Binary, and **NO PAD**: under a
  PAD SPACE collation `'Smith' = 'Smith '` is true, which would silently widen
  `:exact` matching and weaken primary keys. Not `utf8mb4_bin`, which is PAD SPACE.
...
### Independent from fhir-mariadb

The sibling `fhir-mariadb` project began as a fork of this one, and the two are
**independent from here on** (`M14.0a`–`M14.0c`). This port should use whatever
MySQL does best and must not restrict itself to syntax MariaDB also accepts. A
schema installed here is not required to be readable by `fhir-mariadb`, and
...
```sh
scripts/db.sh up      # start pinned MySQL 8.4 in a container
...
`fhir-mysql` handles PHI. It does **not** authenticate or authorize — that is the
