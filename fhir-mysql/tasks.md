...
- [x] **T16 axum skeleton.** `fhir-mysql-server` crate + `fhir-mysql serve`:
...
- [ ] **T22 R3 artifacts.** Same for 3.0.2.
  *Accept:* full R3 examples corpus round-trips live; REST suite green on
  `/r3`.
- [x] **T23 Multi-version serve.** `fhir-mysql serve` mounts every version
...
- [x] **T-graceful.** `fhir-mysql serve` shuts down cleanly on SIGINT/SIGTERM.
...
- [x] **T32 Encrypted database transport (O10.7).** `NoTls` is hard-coded
  (`fhir-mysql-store/src/lib.rs:186`), so PHI crosses to PostgreSQL in clear and
  ...
  *Done:* `SslPolicy`, rustls connector, `PGSSLROOTCERT` trust anchors,
  ...
  whenever the link is unencrypted. fhir-mysql's `require` validates the
...
- [x] **T46 Honest CapabilityStatement (A7.12).** Declare
  ...
  `security.description` stating plainly that fhir-mysql verifies no identity.
