...
Set `FHIR_MYSQL_SPEC_DIR` to point somewhere else. The tests resolve the same two
...
There is no `FHIR_MYSQL_CHAIN_KEY` — an earlier revision of this table
...
coverage this crate has (`mysql_store.rs` builds a `KeyRing` directly with a
