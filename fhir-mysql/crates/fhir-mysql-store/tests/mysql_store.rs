...
async fn fresh(schema: &str) -> Option<MySqlStore> {
    let map = sampled("r5", schema, &["Observation", "Patient"])?;
    let store = MySqlStore::connect(&dsn()?, map).await.expect("connect");
...
async fn connect_fails_loudly_on_a_bad_dsn() {
    ...
    let err = MySqlStore::connect("mysql://root@127.0.0.1:1/", map)
        .await
        .expect_err("connecting to a dead port should fail");
    assert!(
        matches!(err, fhir_mysql_store::StoreError::Db(_)),
...
async fn init_installs_tables_and_triggers() {
    let Some(store) = fresh("fhir_mysql_store_init").await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
...
async fn a_failed_install_reports_how_far_it_got() {
    ...
    // empty or half-built.
    let Some(store) = fresh("fhir_mysql_store_partial").await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
...
async fn put_then_get_round_trips_a_resource() {
    let Some(store) = fresh("fhir_mysql_store_rt").await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
    store.init("rt").await.expect("init");
    let audit = fhir_mysql_store::Audit::default();
    ...
    assert_eq!(put.kind, fhir_mysql_store::mysql::PutKind::Created);
...
async fn rewrite_replaces_children_and_bumps_the_version() {
    let Some(store) = fresh("fhir_mysql_store_rw").await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
    ...
    assert_eq!(put.kind, fhir_mysql_store::mysql::PutKind::Updated);
...
async fn extensions_get_distinct_surrogate_keys() {
    ...
    let Some(store) = fresh("fhir_mysql_store_ext").await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
...
fn test_keys(id: &str) -> fhir_mysql_store::chain::KeyRing {
    let k = fhir_mysql_store::chain::ChainKey::from_hex(
        id,
        "4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f4f",
    )
    .expect("key parses");
    fhir_mysql_store::chain::KeyRing::new(vec![k])
}

async fn fresh_keyed(schema: &str, keys: fhir_mysql_store::chain::KeyRing) -> Option<MySqlStore> {
    let map = sampled("r5", schema, &["Observation", "Patient"])?;
...
async fn history_and_vread_distinguish_a_deletion() {
    let Some(store) = fresh("fhir_mysql_store_hist").await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
...
async fn verify_audit_accepts_a_clean_chain_and_catches_tampering() {
    let Some(store) = fresh_keyed("fhir_mysql_store_verify", test_keys("ci")).await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
    ...
    let raw = MySqlStore::connect(
        &dsn,
        sampled("r5", "fhir_mysql_store_verify", &["Observation"]).expect("map"),
    )
    .await
    .expect("connect");
    raw.exec_raw(
        "DROP TRIGGER `fhir_mysql_store_verify`.`observation_history_append_only_upd_trg_ix`",
    )
    .await
    .expect("drop trigger");
    raw.exec_raw(
        "UPDATE `fhir_mysql_store_verify`.`observation_history` \
...
async fn search_by_token_number_and_paging() {
    let Some(store) = fresh("fhir_mysql_store_search").await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
...
async fn search_values_are_bound_never_interpolated() {
    ...
    let q = fhir_mysql_store::mysql_search::build_search_sql(
        &map,
        rm,
...
async fn purge_erases_history_and_leaves_a_verifiable_hole() {
    let Some(store) = fresh_keyed("fhir_mysql_store_purge", test_keys("ci")).await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
...
async fn history_cannot_be_deleted_without_the_erasure_flag() {
    ...
    let Some(store) = fresh("fhir_mysql_store_flag").await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
    ...
    let err = store
        .exec_raw("DELETE FROM `fhir_mysql_store_flag`.`observation_history` WHERE `id` = 'obs-1'")
        .await
...
async fn disclosures_are_recorded() {
    let Some(store) = fresh("fhir_mysql_store_log").await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
    ...
    let rec = fhir_mysql_store::AccessRecord {
        audit: fhir_mysql_store::Audit {
            actor: "dr-who".into(),
