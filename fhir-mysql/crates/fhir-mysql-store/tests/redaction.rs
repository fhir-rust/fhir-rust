...
use fhir_mysql_store::StoreError;
...
fn sampled(schema: &str) -> Option<Arc<RelMap>> {
    let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../assets/fhir-mysql-relmap-r5.json.gz");
...
async fn phi_reaches_neither_the_log_nor_the_error() {
    let Some(store) = fresh("fhir_mysql_redact_phi").await else {
...
async fn a_rejected_write_names_the_path_without_echoing_the_value() {
    let Some(store) = fresh("fhir_mysql_redact_reject").await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
    ...
    let err = store
        .put(&bad, &fhir_mysql_store::Audit::default())
        .await
