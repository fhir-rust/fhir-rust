...
async fn fresh(schema: &str, want: &[&str]) -> Option<MySqlStore> {
    let store = MySqlStore::connect(&dsn()?, sampled(schema, want)?)
        .await
        .expect("connect");
...
async fn assert_round_trips(store: &MySqlStore, resource: &Value) {
...
/// The F-20 check. `active` is a `boolean`, which MySQL stores as `TINYINT(1)`.
#[tokio::test]
async fn booleans_survive_a_round_trip() {
    let Some(store) = fresh("fhir_mysql_rt_bool", &["Patient"]).await else {
...
async fn a_false_boolean_is_not_confused_with_an_absent_one() {
    let Some(store) = fresh("fhir_mysql_rt_boolf", &["Patient"]).await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
    assert_round_trips(
...
async fn integers_survive_a_round_trip() {
    let Some(store) = fresh("fhir_mysql_rt_int", &["Patient"]).await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
    assert_round_trips(
...
async fn decimal_precision_survives_a_round_trip() {
    let Some(store) = fresh("fhir_mysql_rt_dec", &["Observation"]).await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
    assert_round_trips(
...
async fn partial_dates_survive_a_round_trip() {
    let Some(store) = fresh("fhir_mysql_rt_date", &["Patient"]).await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
    for (id, birth) in [
...
async fn a_resource_mixing_every_column_type_survives() {
    let Some(store) = fresh("fhir_mysql_rt_mixed", &["Patient"]).await else {
        eprintln!("skipping: set FHIR_MYSQL_TEST_DSN to run");
        return;
    };
    assert_round_trips(
