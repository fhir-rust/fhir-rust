...
use fhir_mysql_map::model::{ColTy, RelMap};
...
/// PostgreSQL yields. MySQL binds `Bool` to `TINYINT(1)`, `Int` to `INT`, and
...
impl MySqlStore {
    ...
    /// TLS comes from `FHIR_MYSQL_SSL_MODE` and defaults to verifying
    /// (`O10.7`, **F-54**). Use [`connect_with`](Self::connect_with) to pass a
    /// mode explicitly.
    ///
    /// # Errors
    /// If the DSN is malformed, if `FHIR_MYSQL_SSL_MODE` is not a mode this
    ...
    pub async fn init(&self, checksum: &str) -> Result<usize, StoreError> {
        let statements = fhir_mysql_map::ddl::ddl(&self.map);
        let mut conn = self.pool.get_conn().await.map_err(db_err)?;
        ...
        conn.exec_drop(
            format!(
                "INSERT INTO {schema}.`fhir_mysql_meta` (`key`, `value`) \
    ...
    pub async fn installed_checksum(&self) -> Result<Option<String>, StoreError> {
        ...
        let present: Option<i64> = conn
            .exec_first(
                "SELECT 1 FROM information_schema.tables \
                 WHERE table_schema = ? AND table_name = 'fhir_mysql_meta'",
                (&self.map.schema,),
            )
            .await
            .map_err(db_err)?;
        ...
        conn.query_first(format!(
            "SELECT `value` FROM {schema}.`fhir_mysql_meta` WHERE `key` = 'map_checksum'"
        ))
        .await
        .map_err(db_err)
    }
    ...
    /// `fhir-sqlite`'s, both mysql's doing:
    ///
    /// 1. **There is no transaction.** MySQL commits DDL implicitly, so an
    ...
    /// 2. **The access-log indexes are not idempotent.** MySQL has no
    ...
    ///
    /// **MySQL's failure story is reported-partial** (`M14.38`), because
    ...
    pub async fn upgrade_with(
        ...
        let old_hex: String = conn
            .exec_first(
                format!("SELECT `value` FROM {esc}.`fhir_mysql_meta` WHERE `key` = 'map_asset'"),
                (),
            )
            .await
            .map_err(db_err)?
        ...
        // an element between tables (G2.6a's force-split, F-90) reaches the
                ...
                if let Err(e) = stmt.as_str().ignore(&mut *conn).await {
                    return Err(StoreError::Other(format!(
                        "upgrade: {e}\n\
                         ...
                         — MySQL commits DDL implicitly, so this schema is partial \
                         (spec M14.22, M14.35)\n\
                         statement was:\n{stmt}"
                    )));
                }
        ...
        let mut reconcile: Vec<String> = fhir_mysql_map::ddl::schema_wide_objects(raw)
            .into_iter()
        ...
        for rm in self.map.resources.values() {
            if let Some((_, hist)) = rm.find_table(fhir_mysql_map::model::TableKind::History) {
                let have = self.installed_columns(&mut conn, &s, &hist.name).await?;
                reconcile.extend(
                    fhir_mysql_map::ddl::history_audit_columns(raw, &hist.name)
                        .into_iter()
                ...
                reconcile.extend(fhir_mysql_map::ddl::append_only_triggers(raw, &hist.name));
            }
        }
        ...
        // readable).
        //
        // One InnoDB transaction per resource. MySQL cannot put the *schema*
            ...
            for rtype in rtypes {
                ...
                for id in ids {
                    ...
                    tx.exec_drop(format!("DELETE FROM {esc}.{base_q} WHERE `id` = ?"), (&id,))
                        .await
                        .map_err(db_err)?;
                    let out = fhir_mysql_map::shred::shred(new_rm, &value)?;
                    ...
                    if fhir_mysql_map::canon::canonicalize(&back)
                        != fhir_mysql_map::canon::canonicalize(&value)
                    {
                        return Err(StoreError::Other(format!(
        ...
        for (k, v) in [
            ...
            conn.exec_drop(
                format!("UPDATE {esc}.`fhir_mysql_meta` SET `value` = ? WHERE `key` = ?"),
                (v, k),
            )
            .await
            .map_err(db_err)?;
    ...
    fn diff_maps(
        ...
        let mut old_tables: HashMap<&str, &fhir_mysql_map::model::Table> = HashMap::new();
        ...
        for rm in self.map.resources.values() {
            for t in &rm.tables {
                ...
                let Some(old_t) = old_tables.get(t.name.as_str()) else {
                    adds.push(fhir_mysql_map::ddl::create_table(raw, rm, t));
                    continue;
                };
                ...
                for c in &t.cols {
                    match old_cols.get(c.name.as_str()) {
                        None => adds.push(format!(
                            ...
                            t.name,
                            c.name,
                            fhir_mysql_map::ddl::col_sql(c.ty)
                        )),
        ...
        let old_ix: HashSet<String> = old_map
            .resources
            .values()
            .flat_map(|rm| fhir_mysql_map::ddl::search_indexes(raw, rm))
            .collect();
        for rm in self.map.resources.values() {
            for stmt in fhir_mysql_map::ddl::search_indexes(raw, rm) {
    ...
    pub async fn backfill_norm(&self) -> Result<usize, StoreError> {
        ...
        for (tn, src, dst) in &work {
            loop {
                ...
                for v in &vals {
                    conn.exec_drop(&sql, (fhir_mysql_map::fold::fold(v), v))
                        .await
                        .map_err(db_err)?;
                }
                total += n;
...
fn sqlval(v: &fhir_mysql_map::shred::SqlVal) -> mysql_async::Value {
    use fhir_mysql_map::shred::SqlVal as S;
    use mysql_async::Value as V;
    match v {
...
impl MySqlStore {
    ...
    pub async fn put(
        ...
        let out = fhir_mysql_map::shred::shred(rm, resource)?;
        let id = out
            .id
            .clone()
        ...
        let canon = fhir_mysql_map::canon::canonicalize(resource);
        ...
        let hist = quote_ident(
            &rm.find_table(fhir_mysql_map::model::TableKind::History)
...
fn leaf_from_cols(
    ...
    num: Option<String>,
    b: Option<i64>,
) -> fhir_mysql_map::value::LeafVal {
    use fhir_mysql_map::value::LeafVal;
    match kind {
...
impl MySqlStore {
    ...
    fn hist_target(&self, rtype: &str) -> Result<(String, String), StoreError> {
        let rm = self
            ...
            .ok_or_else(|| StoreError::Unsupported(format!("unknown resource type {rtype}")))?;
        let hist = rm
            .find_table(fhir_mysql_map::model::TableKind::History)
    ...
    pub async fn verify_audit(&self) -> Result<Vec<crate::ChainBreak>, StoreError> {
        use fhir_mysql_map::model::TableKind;
        let s = quote_ident(&self.map.schema);
        let mut conn = self.pool.get_conn().await.map_err(db_err)?;
        ...
        let cs: Vec<mysql_async::Row> = conn
            .query(format!(
                "SELECT `rtype`, `id`, `version_id`, `row_mac` FROM {s}.`fhir_mysql_countersign`"
            ))
            .await
            .map_err(db_err)?;
        for r in cs {
...
impl MySqlStore {
    ...
    pub async fn search_full(
        ...
        count: i64,
        offset: i64,
        sort: &[crate::mysql_search::SortKey],
        want_total: bool,
    ...
    pub async fn search_page(
        ...
        let rm = self
            ...
            .get(rtype)
            .ok_or_else(|| StoreError::Unsupported(format!("unknown resource type {rtype}")))?;
        let q = crate::mysql_search::build_search_sql(
...
impl MySqlStore {
    ...
    pub async fn log_access(&self, rec: &crate::AccessRecord) -> Result<(), StoreError> {
        ...
        conn.exec_drop(
            format!(
                "INSERT INTO {s}.`fhir_mysql_access_log` \
    ...
    pub async fn access_log_len(&self) -> Result<i64, StoreError> {
        ...
        let mut conn = self.pool.get_conn().await.map_err(db_err)?;
        let n: Option<i64> = conn
            .query_first(format!("SELECT count(*) FROM {s}.`fhir_mysql_access_log`"))
            .await
            .map_err(db_err)?;
        Ok(n.unwrap_or(0))
    }
    ...
    /// `@fhir_mysql_erasure` is set. That variable is per-connection, so every
...
async fn write_shredded(
    ...
    s: &str,
    base: &str,
    rm: &fhir_mysql_map::model::ResourceMap,
    id: &str,
    version_id: i64,
    ts: &str,
    out: &fhir_mysql_map::shred::ShredOut,
) -> Result<(), StoreError> {
    use mysql_async::Value as V;
    ...
    let mut by_table: std::collections::BTreeMap<u32, Vec<&fhir_mysql_map::shred::Row>> =
    ...
    if let Some((_, t)) = rm.find_table(fhir_mysql_map::model::TableKind::Ext)
        && !out.ext.is_empty()
    {
        let cols: Vec<String> = [
    ...
    if let Some((_, t)) = rm.find_table(fhir_mysql_map::model::TableKind::Deep)
        && !out.deep.is_empty()
    {
        let cols: Vec<String> = [
    ...
    if let Some((_, t)) = rm.find_table(fhir_mysql_map::model::TableKind::Contained)
...
async fn recon_with_map(
    ...
    use fhir_mysql_map::model::TableKind;
    use fhir_mysql_map::reconstruct::{InRow, ReconIn};
        ...
        match t.kind {
            ...
            TableKind::Ext => {
                let rows: Vec<mysql_async::Row> = tx
                    ...
                    .map_err(db_err)?;
                for r in rows {
                    input.ext.push(fhir_mysql_map::shred::ExtRow {
            ...
            TableKind::Deep => {
                let rows: Vec<mysql_async::Row> = tx
                    ...
                    .map_err(db_err)?;
                for r in rows {
                    input.deep.push(fhir_mysql_map::shred::DeepRow {
                        path: r.get(0).unwrap_or_default(),
                        ords: parse_ords(&r.get::<String, _>(1).unwrap_or_default())?,
    ...
    // (audit F-23).
    let v = fhir_mysql_map::reconstruct::reconstruct(rm, &input, Some(id))?;
    Ok(Some(v))
}
