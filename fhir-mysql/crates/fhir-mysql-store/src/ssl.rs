...
//!
//! # The vocabulary is MySQL's, not libpq's
//!
...
//!
//! `FHIR_MYSQL_SSL_MODE` therefore takes MySQL's own `--ssl-mode` values, and
//! `FHIR_MYSQL_SSL_CA` names a root certificate.
//!
//! # `PREFERRED` is refused, not approximated
//!
//! MySQL's client has a `PREFERRED` mode: encrypt if the server offers it,
...
//! MySQL's own `REQUIRED` does **not** verify — it encrypts and validates
...
/// How this port secures the database connection, in MySQL's vocabulary.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SslMode {
    /// No TLS. Appropriate only for a loopback connection on a host where
    /// nothing else runs.
    Disabled,
    /// TLS required, certificate **not** validated. This is MySQL's own
...
impl SslMode {
    ...
    /// The mode from `FHIR_MYSQL_SSL_MODE`, or the verifying default.
    ///
    /// # Errors
    ...
    pub fn from_env() -> Result<Self, StoreError> {
        match std::env::var("FHIR_MYSQL_SSL_MODE") {
            Ok(v) => Self::parse(&v),
    ...
    ///
    /// `root_ca` is the path from `FHIR_MYSQL_SSL_CA`, for a server whose
...
/// The root-certificate path from `FHIR_MYSQL_SSL_CA`, if set.
#[must_use]
pub fn root_ca_from_env() -> Option<String> {
    std::env::var("FHIR_MYSQL_SSL_CA")
        .ok()
...
mod tests {
    ...
    /// MySQL's `REQUIRED` encrypts without validating, so it must not count as
