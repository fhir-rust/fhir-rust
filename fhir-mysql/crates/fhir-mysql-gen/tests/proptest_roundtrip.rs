...
//!
//! FHIR_MYSQL_PROPTEST_CASES overrides the case count (default 500 locally;
//! CI runs 10000).

use std::collections::HashMap;

use fhir_mysql_map::model::{Elem, ElemKind, Prim, ResourceMap};
...
fn to_recon(rm: &ResourceMap, out: &fhir_mysql_map::ShredOut) -> ReconIn {
...
fn property_roundtrip_random_resources() {
    let spec = std::env::var("FHIR_MYSQL_SPEC_DIR")
    ...
    let cases: u64 = std::env::var("FHIR_MYSQL_PROPTEST_CASES")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(500);
    let map = fhir_mysql_gen::generate(&defs, "r5").expect("generate");
