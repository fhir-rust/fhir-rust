...
//! `fhir-mysql-gen` from the official StructureDefinitions; consumed by the
...
fn bundled_r3() -> Result<RelMap, BundledError> {
    ...
    const GZ: &[u8] = include_bytes!("../assets/fhir-mysql-relmap-r3.json.gz");
...
fn bundled_r4() -> Result<RelMap, BundledError> {
    ...
    // enough to make rustfmt wrap this call differently there and break X15.1's
    // byte-identity for a purely cosmetic reason.
    const GZ: &[u8] = include_bytes!("../assets/fhir-mysql-relmap-r4.json.gz");
    RelMap::from_gz_bytes(GZ).map_err(BundledError::Decode)
}
#[cfg(not(feature = "r4"))]
fn bundled_r4() -> Result<RelMap, BundledError> {
...
fn bundled_r5() -> Result<RelMap, BundledError> {
    ...
    // enough to make rustfmt wrap this call differently there and break X15.1's
    // byte-identity for a purely cosmetic reason.
    const GZ: &[u8] = include_bytes!("../assets/fhir-mysql-relmap-r5.json.gz");
    RelMap::from_gz_bytes(GZ).map_err(BundledError::Decode)
}
#[cfg(not(feature = "r5"))]
fn bundled_r5() -> Result<RelMap, BundledError> {
