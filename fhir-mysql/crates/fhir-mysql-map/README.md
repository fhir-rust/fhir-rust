...
Part of **`fhir-mysql`**, which stores FHIR R3, R4 and R5 resources in MySQL 8.4 as real relational tables — typed columns, child tables, foreign keys, check constraints — and gives them back losslessly.

## Install

```toml
[dependencies]
fhir-mysql-map = "0.4.0"
serde_json = "1"
```
...
```rust
use std::sync::Arc;
use fhir_mysql_map::{RelMap, shred, reconstruct};
...
| `ddl` | **the only dialect-specific file** — emits MySQL DDL |
...
| Column | Purpose |
| --- | --- |
| `<col>_norm` | the folded form, so prefix search is case- and accent-insensitive (`TEXT COLLATE utf8mb4_0900_bin`) |
...
| [Dialect annex](../../spec/14-mysql-dialect.md) | every declared departure from the shared core, by number |
...
**Store** level: native store and search; 102 tests including round-trip-by-type, concurrency, redaction and upgrade+backfill, green against a live MySQL 8.4.
