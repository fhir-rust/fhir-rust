...
| Oracle | **not yet** | the DDL emitter is still MySQL's |
...
Full detail: [conformance matrix](../spec/conformance-matrix.md).
...
PostgreSQL's, but none has a concurrency, redaction, or audit test. Their
guarantees on those points are `?` — plausible, unproven here.
...
One outstanding defect: the hash-chain pre-image is still derived in SQL, so a
PostgreSQL chain cannot be verified by another port
([`audit.md`](../spec/audit.md) **F-07**).
...
- The canonical JSON the hash chain commits to (`X15.2`) — with PostgreSQL the
  outstanding exception (**F-07**).
